...
def __lldb_init_module(debugger, internal_dict):
    ...
    debugger.HandleCommand('type synthetic add -x "^wdDynamicArray<" --python-class wdEngine.wdDynamicArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^wdHybridArray<" --python-class wdEngine.wdHybridArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^wdHybridString<" --python-class wdEngine.wdHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add wdStringBuilder --python-class wdEngine.wdHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add wdStringView --python-class wdEngine.wdStringViewSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^wdEnum<" --python-class wdEngine.wdEnumSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^wdArrayPtr<" --python-class wdEngine.wdArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add wdByteArrayPtr --python-class wdEngine.wdArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add wdConstByteArrayPtr --python-class wdEngine.wdArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^wdMap<" --python-class wdEngine.wdMapSynthProvider')

    debugger.HandleCommand('type summary add -x "^wdHybridString<" --python-function wdEngine.wdHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add wdStringBuilder --python-function wdEngine.wdHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add wdStringView --python-function wdEngine.wdStringView_SummaryProvider')
    debugger.HandleCommand('type summary add -x "^wdEnum<" --python-function wdEngine.wdEnum_SummaryProvider')
...
class wdDynamicArraySynthProvider:
...
class wdHybridArraySynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
...
class wdHybridStringSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
...
def wdHybridString_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "wdHybridString_SummaryProvider"
...
class wdStringViewSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
    ...
    def update(self):
        logger = lldb.formatters.Logger.Logger()
        logger >> "wdStringViewSynthProvider::update"
        self.valid = False
        try:
...
def wdStringView_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "wdStringView_SummaryProvider"
        content = valobj.GetChildAtIndex(0)
        return '"' + make_string(content) + '"'
    except Exception as inst:
        logger >> str(inst)
...
class wdEnumSynthProvider:
    ...
    def update(self):
        self.value = "?"
        logger = lldb.formatters.Logger.Logger()
        logger >> "wdEnumSynthProvider::Update"
        try:
...
def wdEnum_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "wdEnum_SummaryProvider"
        content = valobj.GetChildAtIndex(0)
        return make_string(content)
    except Exception as inst:
        logger >> str(inst)
