...
void wdTelemetry::UpdateNetwork()
{
  ...
  while (true)
  {
    ...
    switch (NetworkEvent.type)
    {
      ...
      {
        if ((wdTelemetry::s_ConnectionMode == wdTelemetry::Server) && (NetworkEvent.peer->eventData != 'EZBC'))
        {
        ...
        else
        {
          ...
          Broadcast(wdTelemetry::Reliable, 'EZBC', 'EZID', &s_uiApplicationID, sizeof(wdUInt32));
...
void wdTelemetry::SendServerName()
{
  ...
  Broadcast(wdTelemetry::Reliable, 'EZBC', 'NAME', data, WD_ARRAY_SIZE(data));
}
