...
struct wdOutputToHTML
{
  static std::ofstream htmlFile;

  static void OutputToHTML(wdTestOutput::Enum type, const char* szMsg)
  {
    if (type != wdTestOutput::StartOutput && !htmlFile.is_open())
      return;
    ...
    switch (type)
    {
      case wdTestOutput::StartOutput:
      {
        ...
        wdTestFramework::GetInstance()->CreateOutputFolder();

        std::string sOutputFile = std::string(wdTestFramework::GetInstance()->GetAbsOutputPath()) + "/UnitTestsLog.htm";
        const char* szTestName = wdTestFramework::GetInstance()->GetTestName();
...
#if WD_ENABLED(WD_PLATFORM_WINDOWS_DESKTOP)
        TestSettings settings = wdTestFramework::GetInstance()->GetSettings();
...
std::ofstream wdOutputToHTML::htmlFile;
