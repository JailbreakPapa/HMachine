...
inline bool SortTest_Operator(const wdTestEntry& lhs, const wdTestEntry& rhs)
{
  return strcmp(lhs.m_szTestName, rhs.m_szTestName) < 0;
}

/// Operator to sort sub-tests alphabetically
inline bool SortSubTest_Operator(const wdSubTestEntry& lhs, const wdSubTestEntry& rhs)
{
  return strcmp(lhs.m_szSubTestName, rhs.m_szSubTestName) < 0;
}

/// Sorts all tests and subtests alphabetically
void SortTestsAlphabetically(std::deque<wdTestEntry>& inout_tests)
{
  std::sort(inout_tests.begin(), inout_tests.end(), SortTest_Operator);

  for (wdUInt32 i = 0; i < inout_tests.size(); ++i)
...
void SaveTestOrder(const char* szFile, const std::deque<wdTestEntry>& allTests)
{
...
/// indented).
inline bool ReadLine(FILE* pFile, char* pDest, wdUInt32 uiBufferSize)
{
  wdUInt32 iPos = 0;
...
void LoadTestOrder(const char* szFile, std::deque<wdTestEntry>& ref_allTests)
{
  ...
  for (wdTestEntry& test : ref_allTests)
  {
    test.m_bEnableTest = false;
    for (wdSubTestEntry& subTest : test.m_SubTests)
    {
      subTest.m_bEnableTest = false;
    }
  }

  wdInt32 iLastMainTest = 0;
  ...
  while (!feof(pFile))
  {
    ...
    if (pEnd)
    {
      wdInt32 iPos = (wdInt32)(pEnd - szTestName);
      szTestName[iPos] = '\0';
    }
    ...
    if (szTestName[0] != '\0')
    {
      if (!bIndented)
      {
        ...
        for (wdUInt32 t = 0; t < ref_allTests.size(); ++t)
        {
      ...
      else
      {
        ...
        if (iLastMainTest >= 0)
        {
          for (wdUInt32 t = 0; t < ref_allTests[iLastMainTest].m_SubTests.size(); ++t)
          {
...
WD_STATICLINK_FILE(TestFramework, TestFramework_Utilities_TestOrder);
