...
struct wdTestEntry
{
  ...
  wdTestBaseClass* m_pTest;
  const char* m_szTestName;
  std::deque<wdSubTestEntry> m_SubTests;
...
struct TestSettings
{
  ...
  wdUInt8 m_uiFullPasses = 1;        /// All tests are done this often, to check whether some tests fail only when executed multiple times.
};
