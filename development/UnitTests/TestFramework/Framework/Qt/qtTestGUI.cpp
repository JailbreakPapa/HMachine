...
////////////////////////////////////////////////////////////////////////
// wdQtTestGUI public functions
////////////////////////////////////////////////////////////////////////

wdQtTestGUI::wdQtTestGUI(wdQtTestFramework& ref_testFramework)
  : QMainWindow()
  , m_pTestFramework(&ref_testFramework)
{
  ...
  QCoreApplication::setOrganizationDomain("www.wdengine.net");
  QCoreApplication::setOrganizationName("wdEngine Project");
  QCoreApplication::setApplicationName("wdTestFramework");
  ...
  // Model
  m_pModel = new wdQtTestModel(this, m_pTestFramework);
  testTreeView->setModel(m_pModel);

  // Delegate
  m_pDelegate = new wdQtTestDelegate(this);
  ...
  // Message Log Dock
  m_pMessageLogDock = new wdQtLogMessageDock(this, &m_pTestFramework->GetTestResult());
...
void wdQtTestGUI::closeEvent(QCloseEvent* e)
{
...
////////////////////////////////////////////////////////////////////////
// wdQtTestGUI public slots
////////////////////////////////////////////////////////////////////////

void wdQtTestGUI::on_actionAssertOnTestFail_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void wdQtTestGUI::on_actionOpenHTMLOutput_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void wdQtTestGUI::on_actionKeepConsoleOpen_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void wdQtTestGUI::on_actionShowMessageBox_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void wdQtTestGUI::on_actionDisableSuccessfulTests_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void wdQtTestGUI::on_actionSaveTestSettingsAs_triggered()
{
  wdStringBuilder tmp;
  wdStringView defaultDir = wdPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
...
void wdQtTestGUI::on_actionSaveTestOrderAs_triggered()
{
  wdStringBuilder tmp;
  wdStringView defaultDir = wdPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
  QString dir = defaultDir.IsValid() ? defaultDir.GetData(tmp) : QString();
...
void wdQtTestGUI::on_actionRunTests_triggered()
{
  ...
  while (m_pTestFramework->RunTestExecutionLoop() == wdTestAppRun::Continue)
  {
    UpdateButtonStates();
...
void wdQtTestGUI::on_actionAbort_triggered()
{
  m_bAbort = true;
}

void wdQtTestGUI::on_actionQuit_triggered()
{
  close();
}

void wdQtTestGUI::on_actionEnableOnlyThis_triggered()
{
...
void wdQtTestGUI::on_actionEnableOnlyFailed_triggered()
{
  m_pTestFramework->SetAllFailedTestsEnabledStatus();
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void wdQtTestGUI::on_actionEnableAllChildren_triggered()
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
  if (!CurrentIndex.isValid())
    return;
...
void wdQtTestGUI::on_actionEnableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(true);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void wdQtTestGUI::on_actionDisableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(false);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void wdQtTestGUI::on_actionExpandAll_triggered()
{
  testTreeView->expandAll();
}

void wdQtTestGUI::on_actionCollapseAll_triggered()
{
  testTreeView->collapseAll();
}

void wdQtTestGUI::onTestFrameworkTestResultReceived(qint32 iTestIndex, qint32 iSubTestIndex)
{
  ...
  // Update status bar
  const wdUInt32 uiTestCount = m_uiTestsEnabledCount;
  const wdUInt32 uiFailed = m_pTestFramework->GetTestsFailedCount();
  const wdUInt32 uiPassed = m_pTestFramework->GetTestsPassedCount();
  const wdUInt32 uiErrors = m_pTestFramework->GetTotalErrorCount();
  ...
  {
    fSubTestPercentage =
      (float)m_pTestFramework->GetTestResult().GetSubTestCount(m_pTestFramework->GetCurrentTestIndex(), wdTestResultQuery::Executed) /
...
void wdQtTestGUI::onTestTreeViewCustomContextMenuRequested(const QPoint& pnt)
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
...
void wdQtTestGUI::onSelectionModelCurrentRowChanged(const QModelIndex& index)
{
  if (!index.isValid())
  {
  ...
  const wdTestResultData* pTestResult = pEntry->GetTestResult();
...
void wdQtTestGUI::on_actionOpenTestDataFolder_triggered()
{
...
void wdQtTestGUI::on_actionOpenOutputFolder_triggered()
{
...
void wdQtTestGUI::on_actionOpenHTMLFile_triggered()
{
  std::string sOutputFile = std::string(wdTestFramework::GetInstance()->GetAbsOutputPath()) + "/UnitTestsLog.htm";

  QDesktopServices::openUrl(QUrl::fromLocalFile(sOutputFile.c_str()));
}

void wdQtTestGUI::on_actionUpdateReferenceImages_triggered()
{
  m_pTestFramework->UpdateReferenceImages();
}

////////////////////////////////////////////////////////////////////////
// wdQtTestGUI private functions
////////////////////////////////////////////////////////////////////////

void wdQtTestGUI::UpdateButtonStates()
{
...
void wdQtTestGUI::SaveGUILayout()
{
  QSettings Settings;
...
void wdQtTestGUI::LoadGUILayout()
{
  QSettings Settings;
  Settings.beginGroup("MainWindow");
...
void wdQtTestGUI::SetCheckStateRecursive(const QModelIndex& index, bool bChecked)
{
  m_pModel->setData(index, bChecked ? Qt::Checked : Qt::Unchecked, Qt::CheckStateRole);

  wdInt32 iChildren = m_pModel->rowCount(index);
...
void wdQtTestGUI::EnableAllParents(const QModelIndex& index)
{
...
////////////////////////////////////////////////////////////////////////
// wdQtTestGUI public static functions
////////////////////////////////////////////////////////////////////////

void wdQtTestGUI::SetDarkTheme()
{
  // return;
...
WD_STATICLINK_FILE(TestFramework, TestFramework_Framework_Qt_qtTestGUI);
