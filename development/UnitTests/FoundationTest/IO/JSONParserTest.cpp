...
class TestReader : public wdJSONParser
{
private:
  wdDeque<ParseResult> m_Results;
...
public:
  ...
  ~TestReader()
  {
    WD_TEST_INT(m_iExpectedParsingErrors, 0);
    WD_TEST_INT(m_Results.GetCount(), 0);
  }

  void ParseStream(wdStreamReader& inout_stream)
  {
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == Variable);
    WD_TEST_STRING(m_Results.PeekFront().m_szValue, szVarName);

    m_Results.PopFront();

    m_bSkipObject = wdStringUtils::IsEqual(szVarName, "skip_obj");
    m_bSkipArray = wdStringUtils::IsEqual(szVarName, "skip_array");

    return !wdStringUtils::IsEqual(szVarName, "skip_var");
  }

  virtual void OnReadValue(const char* szValue) override
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == ValueString);
    WD_TEST_STRING(m_Results.PeekFront().m_szValue, szValue);
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == ValueDouble);
    WD_TEST_DOUBLE(m_Results.PeekFront().m_fValue, fValue, 0.0001);
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == ValueBool);
    WD_TEST_BOOL(m_Results.PeekFront().m_bValue == bValue);
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == ValueNULL);
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == BeginObject);
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == EndObject);
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == BeginArray);
  ...
  {
    WD_TEST_BOOL(!m_Results.IsEmpty());
    WD_TEST_BOOL(m_Results.PeekFront().m_Function == EndArray);

    m_Results.PopFront();
  }

  wdInt32 m_iExpectedParsingErrors;
  ...
  virtual void OnParsingError(const char* szMessage, bool bFatal, wdUInt32 uiLine, wdUInt32 uiColumn) override
  {
    ...
    WD_TEST_FAILURE("JSON Parsing Error", "(%u, %u): %s", uiLine, uiColumn, szMessage);
  }
...
WD_CREATE_SIMPLE_TEST(IO, JSONParser)
{
  WD_TEST_BLOCK(wdTestBlock::Enabled, "All Features")
  {
...
\"test\" : \"text\"\n\
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Empty Document (string")
  {
    const char* szTestData = "";
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Empty Document (Whitespace)")
  {
    const char* szTestData = " \n  \t ";
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Empty Document")
  {
    const char* szTestData = "{}";
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Two Empty Documents")
  {
    const char* szTestData = "{}{}";
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "No Whitespace")
  {
    ...
    WD_TEST_INT(reader.m_iExpectedParsingErrors, 0);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Empty Blocks")
  {
    // I want C++ 11 raw string literals
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Superfluous separators")
  {
    // I want C++ 11 raw string literals
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Too many commas in array")
  {
    // I want C++ 11 raw string literals
    ...
    WD_TEST_INT(reader.m_iExpectedParsingErrors, 0); // must fail to parse
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Comments")
  {
    // I want C++ 11 raw string literals
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Skip Object")
  {
    // I want C++ 11 raw string literals
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Skip Array")
  {
    // I want C++ 11 raw string literals
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Skip Variable")
  {
    // I want C++ 11 raw string literals
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Unicode escape sequence")
  {
  ...
  WD_TEST_BLOCK(wdTestBlock::Enabled, "Unicode escape sequence parsing errors")
  {
    const char* szTestData = "{\"a\":\"\\u006G\",\"b\":\"\\u03f\",\"c\":\"a\\ud83wd\",\"d\":\"\\ud83e\t\"}";
