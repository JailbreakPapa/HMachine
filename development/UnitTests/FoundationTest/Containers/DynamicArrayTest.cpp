#include <FoundationTest/FoundationTestPCH.h>

#include <Foundation/Containers/DynamicArray.h>
#include <Foundation/Memory/CommonAllocators.h>
#include <Foundation/Time/Stopwatch.h>
#include <Foundation/Types/UniquePtr.h>

namespace DynamicArrayTestDetail
{
  typedef wdConstructionCounter st;

  static int g_iDummyCounter = 0;

  class Dummy
  {
  public:
    int a;
    int b;
    std::string s;

    Dummy()
      : a(0)
      , b(g_iDummyCounter++)
      , s("Test")
    {
    }
    Dummy(int a)
      : a(a)
      , b(g_iDummyCounter++)
      , s("Test")
    {
    }

    bool operator<=(const Dummy& dummy) const { return a <= dummy.a; }
    bool operator>=(const Dummy& dummy) const { return a >= dummy.a; }
    bool operator>(const Dummy& dummy) const { return a > dummy.a; }
    bool operator<(const Dummy& dummy) const { return a < dummy.a; }
    bool operator==(const Dummy& dummy) const { return a == dummy.a; }
  };

  wdAllocatorBase* g_pTestAllocator;

  struct wdTestAllocatorWrapper
  {
    static wdAllocatorBase* GetAllocator() { return g_pTestAllocator; }
  };

  template <typename T = st, typename AllocatorWrapper = wdTestAllocatorWrapper>
  static wdDynamicArray<T, AllocatorWrapper> CreateArray(wdUInt32 uiSize, wdUInt32 uiOffset)
  {
    wdDynamicArray<T, AllocatorWrapper> a;
    a.SetCount(uiSize);

    for (wdUInt32 i = 0; i < uiSize; ++i)
      a[i] = T(uiOffset + i);

    return a;
  }
} // namespace DynamicArrayTestDetail

#if WD_ENABLED(WD_PLATFORM_64BIT)
WD_CHECK_AT_COMPILETIME(sizeof(wdDynamicArray<wdInt32>) == 24);
#else
WD_CHECK_AT_COMPILETIME(sizeof(wdDynamicArray<wdInt32>) == 16);
#endif

WD_CREATE_SIMPLE_TEST_GROUP(Containers);

WD_CREATE_SIMPLE_TEST(Containers, DynamicArray)
{
  wdProxyAllocator proxy("DynamicArrayTestAllocator", wdFoundation::GetDefaultAllocator());
  DynamicArrayTestDetail::g_pTestAllocator = &proxy;

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Constructor")
  {
    wdDynamicArray<wdInt32> a1;
    wdDynamicArray<DynamicArrayTestDetail::st> a2;

    WD_TEST_BOOL(a1.GetCount() == 0);
    WD_TEST_BOOL(a2.GetCount() == 0);
    WD_TEST_BOOL(a1.IsEmpty());
    WD_TEST_BOOL(a2.IsEmpty());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Copy Constructor")
  {
    wdDynamicArray<wdInt32, DynamicArrayTestDetail::wdTestAllocatorWrapper> a1;

    WD_TEST_BOOL(a1.GetHeapMemoryUsage() == 0);

    for (wdInt32 i = 0; i < 32; ++i)
      a1.PushBack(rand() % 100000);

    WD_TEST_BOOL(a1.GetHeapMemoryUsage() >= 32 * sizeof(wdInt32));

    wdDynamicArray<wdInt32> a2 = a1;
    wdDynamicArray<wdInt32> a3(a1);

    WD_TEST_BOOL(a1 == a2);
    WD_TEST_BOOL(a1 == a3);
    WD_TEST_BOOL(a2 == a3);

    wdInt32 test[] = {1, 2, 3, 4};
    wdArrayPtr<wdInt32> aptr(test);

    wdDynamicArray<wdInt32> a4(aptr);

    WD_TEST_BOOL(a4 == aptr);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Move Constructor / Operator")
  {
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

    {
      // move constructor
      wdDynamicArray<DynamicArrayTestDetail::st, DynamicArrayTestDetail::wdTestAllocatorWrapper> a1(DynamicArrayTestDetail::CreateArray(100, 20));

      WD_TEST_INT(a1.GetCount(), 100);
      for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
        WD_TEST_INT(a1[i].m_iData, 20 + i);

      // move operator
      a1 = DynamicArrayTestDetail::CreateArray(200, 50);

      WD_TEST_INT(a1.GetCount(), 200);
      for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
        WD_TEST_INT(a1[i].m_iData, 50 + i);
    }

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

    {
      // move assignment with different allocators
      wdConstructionCounterRelocatable::Reset();
      wdProxyAllocator proxyAllocator("test allocator", wdFoundation::GetDefaultAllocator());
      {
        wdDynamicArray<wdConstructionCounterRelocatable> a1(&proxyAllocator);

        a1 = DynamicArrayTestDetail::CreateArray<wdConstructionCounterRelocatable, wdDefaultAllocatorWrapper>(8, 70);
        WD_TEST_BOOL(wdConstructionCounterRelocatable::HasDone(8, 0));
        WD_TEST_BOOL(a1.GetAllocator() == &proxyAllocator); // allocator must not change

        WD_TEST_INT(a1.GetCount(), 8);
        for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
          WD_TEST_INT(a1[i].m_iData, 70 + i);

        a1 = DynamicArrayTestDetail::CreateArray<wdConstructionCounterRelocatable, wdDefaultAllocatorWrapper>(32, 100);
        WD_TEST_BOOL(wdConstructionCounterRelocatable::HasDone(32, 8));
        WD_TEST_BOOL(a1.GetAllocator() == &proxyAllocator); // allocator must not change

        WD_TEST_INT(a1.GetCount(), 32);
        for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
          WD_TEST_INT(a1[i].m_iData, 100 + i);
      }

      WD_TEST_BOOL(wdConstructionCounterRelocatable::HasAllDestructed());
      wdConstructionCounterRelocatable::Reset();

      auto allocatorStats = proxyAllocator.GetStats();
      WD_TEST_BOOL(allocatorStats.m_uiNumAllocations == allocatorStats.m_uiNumDeallocations); // check for memory leak?
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Convert to ArrayPtr")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 100; ++i)
    {
      wdInt32 r = rand() % 100000;
      a1.PushBack(r);
    }

    wdArrayPtr<wdInt32> ap = a1;

    WD_TEST_BOOL(ap.GetCount() == a1.GetCount());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "operator =")
  {
    wdDynamicArray<wdInt32, DynamicArrayTestDetail::wdTestAllocatorWrapper> a1;
    wdDynamicArray<wdInt32> a2;

    for (wdInt32 i = 0; i < 100; ++i)
      a1.PushBack(i);

    a2 = a1;

    WD_TEST_BOOL(a1 == a2);

    wdArrayPtr<wdInt32> arrayPtr(a1);

    a2 = arrayPtr;

    WD_TEST_BOOL(a2 == arrayPtr);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "operator == / !=/ <")
  {
    wdDynamicArray<wdInt32> a1, a2;

    WD_TEST_BOOL(a1 == a1);
    WD_TEST_BOOL(a2 == a2);
    WD_TEST_BOOL(a1 == a2);

    WD_TEST_BOOL((a1 != a1) == false);
    WD_TEST_BOOL((a2 != a2) == false);
    WD_TEST_BOOL((a1 != a2) == false);

    for (wdInt32 i = 0; i < 100; ++i)
    {
      wdInt32 r = rand() % 100000;
      a1.PushBack(r);
      a2.PushBack(r);
    }

    WD_TEST_BOOL(a1 == a1);
    WD_TEST_BOOL(a2 == a2);
    WD_TEST_BOOL(a1 == a2);

    WD_TEST_BOOL((a1 != a2) == false);

    WD_TEST_BOOL((a1 < a2) == false);
    a2.PushBack(100);
    WD_TEST_BOOL(a1 < a2);
    a1.PushBack(99);
    WD_TEST_BOOL(a1 < a2);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Index operator")
  {
    wdDynamicArray<wdInt32> a1;
    a1.SetCountUninitialized(100);

    for (wdInt32 i = 0; i < 100; ++i)
      a1[i] = i;

    for (wdInt32 i = 0; i < 100; ++i)
      WD_TEST_INT(a1[i], i);

    wdDynamicArray<wdInt32> ca1;
    ca1 = a1;

    for (wdInt32 i = 0; i < 100; ++i)
      WD_TEST_INT(ca1[i], i);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SetCount / GetCount / IsEmpty")
  {
    wdDynamicArray<wdInt32> a1;

    WD_TEST_BOOL(a1.IsEmpty());

    for (wdInt32 i = 0; i < 128; ++i)
    {
      a1.SetCount(i + 1);
      WD_TEST_INT(a1[i], 0);
      a1[i] = i;

      WD_TEST_INT(a1.GetCount(), i + 1);
      WD_TEST_BOOL(!a1.IsEmpty());
    }

    for (wdInt32 i = 0; i < 128; ++i)
      WD_TEST_INT(a1[i], i);

    for (wdInt32 i = 128; i >= 0; --i)
    {
      a1.SetCount(i);

      WD_TEST_INT(a1.GetCount(), i);

      for (wdInt32 i2 = 0; i2 < i; ++i2)
        WD_TEST_INT(a1[i2], i2);
    }

    WD_TEST_BOOL(a1.IsEmpty());

    a1.SetCountUninitialized(32);
    WD_TEST_INT(a1.GetCount(), 32);
    a1[31] = 45;
    WD_TEST_INT(a1[31], 45);

    // Test SetCount with fill value
    {
      wdDynamicArray<wdInt32> a2;
      a2.PushBack(5);
      a2.PushBack(3);
      a2.SetCount(10, 42);

      if (WD_TEST_INT(a2.GetCount(), 10))
      {
        WD_TEST_INT(a2[0], 5);
        WD_TEST_INT(a2[1], 3);
        WD_TEST_INT(a2[4], 42);
        WD_TEST_INT(a2[9], 42);
      }

      a2.Clear();
      a2.PushBack(1);
      a2.PushBack(2);
      a2.PushBack(3);

      a2.SetCount(2, 10);
      if (WD_TEST_INT(a2.GetCount(), 2))
      {
        WD_TEST_INT(a2[0], 1);
        WD_TEST_INT(a2[1], 2);
      }
    }
  }

  // Test SetCount with fill value
  {
    wdDynamicArray<wdInt32> a2;
    a2.PushBack(5);
    a2.PushBack(3);
    a2.SetCount(10, 42);

    if (WD_TEST_INT(a2.GetCount(), 10))
    {
      WD_TEST_INT(a2[0], 5);
      WD_TEST_INT(a2[1], 3);
      WD_TEST_INT(a2[4], 42);
      WD_TEST_INT(a2[9], 42);
    }

    a2.Clear();
    a2.PushBack(1);
    a2.PushBack(2);
    a2.PushBack(3);

    a2.SetCount(2, 10);
    if (WD_TEST_INT(a2.GetCount(), 2))
    {
      WD_TEST_INT(a2[0], 1);
      WD_TEST_INT(a2[1], 2);
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "EnsureCount")
  {
    wdDynamicArray<wdInt32> a1;

    WD_TEST_INT(a1.GetCount(), 0);

    a1.EnsureCount(0);
    WD_TEST_INT(a1.GetCount(), 0);

    a1.EnsureCount(1);
    WD_TEST_INT(a1.GetCount(), 1);

    a1.EnsureCount(2);
    WD_TEST_INT(a1.GetCount(), 2);

    a1.EnsureCount(1);
    WD_TEST_INT(a1.GetCount(), 2);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Clear")
  {
    wdDynamicArray<wdInt32> a1;
    a1.Clear();

    a1.PushBack(3);
    a1.Clear();

    WD_TEST_BOOL(a1.IsEmpty());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Contains / IndexOf / LastIndexOf")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = -100; i < 100; ++i)
      WD_TEST_BOOL(!a1.Contains(i));

    for (wdInt32 i = 0; i < 100; ++i)
      a1.PushBack(i);
    for (wdInt32 i = 0; i < 100; ++i)
      a1.PushBack(i);

    for (wdInt32 i = 0; i < 100; ++i)
    {
      WD_TEST_BOOL(a1.Contains(i));
      WD_TEST_INT(a1.IndexOf(i), i);
      WD_TEST_INT(a1.IndexOf(i, 100), i + 100);
      WD_TEST_INT(a1.LastIndexOf(i), i + 100);
      WD_TEST_INT(a1.LastIndexOf(i, 100), i);
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "PushBackUnchecked / PushBackRange")
  {
    wdDynamicArray<wdInt32> a1;
    a1.Reserve(100);

    for (wdInt32 i = 0; i < 100; ++i)
      a1.PushBackUnchecked(i);

    for (wdInt32 i = 0; i < 100; ++i)
      WD_TEST_INT(a1[i], i);

    wdInt32 temp[] = {100, 101, 102, 103, 104};
    wdArrayPtr<wdInt32> range(temp);

    a1.PushBackRange(range);

    WD_TEST_INT(a1.GetCount(), 105);
    for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
      WD_TEST_INT(a1[i], i);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Insert")
  {
    wdDynamicArray<wdInt32> a1;

    // always inserts at the front
    for (wdInt32 i = 0; i < 100; ++i)
      a1.Insert(i, 0);

    for (wdInt32 i = 0; i < 100; ++i)
      WD_TEST_INT(a1[i], 99 - i);

    wdUniquePtr<DynamicArrayTestDetail::st> ptr = WD_DEFAULT_NEW(DynamicArrayTestDetail::st);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasConstructed(1));

    {
      wdDynamicArray<wdUniquePtr<DynamicArrayTestDetail::st>> a2;
      for (wdUInt32 i = 0; i < 10; ++i)
        a2.Insert(wdUniquePtr<DynamicArrayTestDetail::st>(), 0);

      a2.Insert(std::move(ptr), 0);
      WD_TEST_BOOL(ptr == nullptr);
      WD_TEST_BOOL(a2[0] != nullptr);

      for (wdUInt32 i = 1; i < a2.GetCount(); ++i)
        WD_TEST_BOOL(a2[i] == nullptr);
    }

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "InsertRange")
  {
    // Pod element tests
    wdDynamicArray<wdInt32> intTestRange;
    wdDynamicArray<wdInt32> a1;

    wdInt32 intTemp1[] = {91, 92, 93, 94, 95};
    wdArrayPtr<wdInt32> intRange1(intTemp1);

    wdInt32 intTemp2[] = {96, 97, 98, 99, 100};
    wdArrayPtr<wdInt32> intRange2(intTemp2);

    wdInt32 intTemp3[] = {100, 101, 102, 103, 104};
    wdArrayPtr<wdInt32> intRange3(intTemp3);

    {
      intTestRange.PushBackRange(intRange3);

      a1.InsertRange(intRange3, 0);

      WD_TEST_INT(a1.GetCount(), 5);

      for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
        WD_TEST_INT(a1[i], intTestRange[i]);
    }

    {
      intTestRange.Clear();
      intTestRange.PushBackRange(intRange1);
      intTestRange.PushBackRange(intRange3);

      a1.InsertRange(intRange1, 0);

      WD_TEST_INT(a1.GetCount(), 10);
      for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
        WD_TEST_INT(a1[i], intTestRange[i]);
    }

    {
      intTestRange.Clear();
      intTestRange.PushBackRange(intRange1);
      intTestRange.PushBackRange(intRange2);
      intTestRange.PushBackRange(intRange3);

      a1.InsertRange(intRange2, 5);

      WD_TEST_INT(a1.GetCount(), 15);
      for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
        WD_TEST_INT(a1[i], intTestRange[i]);
    }

    // Class element tests
    wdDynamicArray<wdDeque<wdString>> classTestRange;
    wdDynamicArray<wdDeque<wdString>> a2;

    wdDeque<wdString> strTemp1[4];
    {
      strTemp1[0].PushBack("One");
      strTemp1[1].PushBack("Two");
      strTemp1[2].PushBack("Three");
      strTemp1[3].PushBack("Four");
    }
    wdArrayPtr<wdDeque<wdString>> classRange1(strTemp1);

    wdDeque<wdString> strTemp2[3];
    {
      strTemp2[0].PushBack("Five");
      strTemp2[1].PushBack("Six");
      strTemp2[2].PushBack("Seven");
    }
    wdArrayPtr<wdDeque<wdString>> classRange2(strTemp2);

    wdDeque<wdString> strTemp3[3];
    {
      strTemp3[0].PushBack("Eight");
      strTemp3[1].PushBack("Nine");
      strTemp3[2].PushBack("Ten");
    }
    wdArrayPtr<wdDeque<wdString>> classRange3(strTemp3);

    {
      classTestRange.PushBackRange(classRange3);

      a2.InsertRange(classRange3, 0);

      WD_TEST_INT(a2.GetCount(), 3);

      for (wdUInt32 i = 0; i < a2.GetCount(); ++i)
        WD_TEST_STRING(a2[i].PeekFront(), classTestRange[i].PeekFront());
    }

    {
      classTestRange.Clear();
      classTestRange.PushBackRange(classRange1);
      classTestRange.PushBackRange(classRange3);

      a2.InsertRange(classRange1, 0);

      WD_TEST_INT(a2.GetCount(), 7);
      for (wdUInt32 i = 0; i < a2.GetCount(); ++i)
        WD_TEST_STRING(a2[i].PeekFront(), classTestRange[i].PeekFront());
    }

    {
      classTestRange.Clear();
      classTestRange.PushBackRange(classRange1);
      classTestRange.PushBackRange(classRange2);
      classTestRange.PushBackRange(classRange3);

      a2.InsertRange(classRange2, 4);

      WD_TEST_INT(a2.GetCount(), 10);
      for (wdUInt32 i = 0; i < a2.GetCount(); ++i)
        WD_TEST_STRING(a2[i].PeekFront(), classTestRange[i].PeekFront());
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "RemoveAndCopy")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 100; ++i)
      a1.PushBack(i % 2);

    while (a1.RemoveAndCopy(1))
    {
    }

    WD_TEST_BOOL(a1.GetCount() == 50);

    for (wdUInt32 i = 0; i < a1.GetCount(); ++i)
      WD_TEST_INT(a1[i], 0);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "RemoveAndSwap")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 10; ++i)
      a1.Insert(i, i); // inserts at the end

    a1.RemoveAndSwap(9);
    a1.RemoveAndSwap(7);
    a1.RemoveAndSwap(5);
    a1.RemoveAndSwap(3);
    a1.RemoveAndSwap(1);

    WD_TEST_INT(a1.GetCount(), 5);

    for (wdInt32 i = 0; i < 5; ++i)
      WD_TEST_BOOL(wdMath::IsEven(a1[i]));
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "RemoveAtAndCopy")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 10; ++i)
      a1.Insert(i, i); // inserts at the end

    a1.RemoveAtAndCopy(9);
    a1.RemoveAtAndCopy(7);
    a1.RemoveAtAndCopy(5);
    a1.RemoveAtAndCopy(3);
    a1.RemoveAtAndCopy(1);

    WD_TEST_INT(a1.GetCount(), 5);

    for (wdInt32 i = 0; i < 5; ++i)
      WD_TEST_INT(a1[i], i * 2);

    wdUniquePtr<DynamicArrayTestDetail::st> ptr = WD_DEFAULT_NEW(DynamicArrayTestDetail::st);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasConstructed(1));

    {
      wdDynamicArray<wdUniquePtr<DynamicArrayTestDetail::st>> a2;
      for (wdUInt32 i = 0; i < 10; ++i)
        a2.Insert(wdUniquePtr<DynamicArrayTestDetail::st>(), 0);

      a2.PushBack(std::move(ptr));
      WD_TEST_BOOL(ptr == nullptr);
      WD_TEST_BOOL(a2[10] != nullptr);

      a2.RemoveAtAndCopy(0);
      WD_TEST_BOOL(a2[9] != nullptr);
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 0));
    }

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "RemoveAtAndSwap")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 10; ++i)
      a1.Insert(i, i); // inserts at the end

    a1.RemoveAtAndSwap(9);
    a1.RemoveAtAndSwap(7);
    a1.RemoveAtAndSwap(5);
    a1.RemoveAtAndSwap(3);
    a1.RemoveAtAndSwap(1);

    WD_TEST_INT(a1.GetCount(), 5);

    for (wdInt32 i = 0; i < 5; ++i)
      WD_TEST_BOOL(wdMath::IsEven(a1[i]));

    wdUniquePtr<DynamicArrayTestDetail::st> ptr = WD_DEFAULT_NEW(DynamicArrayTestDetail::st);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasConstructed(1));

    {
      wdDynamicArray<wdUniquePtr<DynamicArrayTestDetail::st>> a2;
      for (wdUInt32 i = 0; i < 10; ++i)
        a2.Insert(wdUniquePtr<DynamicArrayTestDetail::st>(), 0);

      a2.PushBack(std::move(ptr));
      WD_TEST_BOOL(ptr == nullptr);
      WD_TEST_BOOL(a2[10] != nullptr);

      a2.RemoveAtAndSwap(0);
      WD_TEST_BOOL(a2[0] != nullptr);
    }

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "PushBack / PopBack / PeekBack")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 10; ++i)
    {
      a1.PushBack(i);
      WD_TEST_INT(a1.PeekBack(), i);
    }

    for (wdInt32 i = 9; i >= 0; --i)
    {
      WD_TEST_INT(a1.PeekBack(), i);
      a1.PopBack();
    }

    a1.PushBack(23);
    a1.PushBack(2);
    a1.PushBack(3);

    a1.PopBack(2);
    WD_TEST_INT(a1.PeekBack(), 23);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "ExpandAndGetRef")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 20; ++i)
    {
      wdInt32& intRef = a1.ExpandAndGetRef();
      intRef = i * 5;
    }


    WD_TEST_BOOL(a1.GetCount() == 20);

    for (wdInt32 i = 0; i < 20; ++i)
    {
      WD_TEST_INT(a1[i], i * 5);
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Construction / Destruction")
  {
    {
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

      wdDynamicArray<DynamicArrayTestDetail::st> a1;
      wdDynamicArray<DynamicArrayTestDetail::st> a2;

      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

      a1.PushBack(DynamicArrayTestDetail::st(1));
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(2, 1)); // one temporary, one final (copy constructed)

      a1.Insert(DynamicArrayTestDetail::st(2), 0);
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(2, 1)); // one temporary, one final (copy constructed)

      a2 = a1;
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(2, 0)); // two copies

      a1.Clear();
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 2));

      a1.PushBack(DynamicArrayTestDetail::st(3));
      a1.PushBack(DynamicArrayTestDetail::st(4));
      a1.PushBack(DynamicArrayTestDetail::st(5));
      a1.PushBack(DynamicArrayTestDetail::st(6));

      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(8, 4)); // four temporaries

      a1.RemoveAndCopy(DynamicArrayTestDetail::st(3));
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(1, 2)); // one temporary, one destroyed

      a1.RemoveAndCopy(DynamicArrayTestDetail::st(3));
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(1, 1)); // one temporary, none destroyed

      a1.RemoveAtAndCopy(0);
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 1)); // one destroyed

      a1.RemoveAtAndSwap(0);
      WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 1)); // one destroyed
    }

    // tests the destructor of a2 and a1
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SortingPrimitives")
  {
    wdDynamicArray<wdUInt32> list;

    list.Sort();

    for (wdUInt32 i = 0; i < 450; i++)
    {
      list.PushBack(std::rand());
    }
    list.Sort();

    for (wdUInt32 i = 1; i < list.GetCount(); i++)
    {
      WD_TEST_BOOL(list[i - 1] <= list[i]);
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SortingObjects")
  {
    wdDynamicArray<DynamicArrayTestDetail::Dummy> list;
    list.Reserve(128);

    for (wdUInt32 i = 0; i < 100; i++)
    {
      list.PushBack(DynamicArrayTestDetail::Dummy(rand()));
    }
    list.Sort();

    for (wdUInt32 i = 1; i < list.GetCount(); i++)
    {
      WD_TEST_BOOL(list[i - 1] <= list[i]);
      WD_TEST_BOOL(list[i].s == "Test");
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SortingMovableObjects")
  {
    {
      wdDynamicArray<wdUniquePtr<DynamicArrayTestDetail::st>> list;
      list.Reserve(128);

      for (wdUInt32 i = 0; i < 100; i++)
      {
        list.PushBack(WD_DEFAULT_NEW(DynamicArrayTestDetail::st));
      }
      list.Sort();

      for (wdUInt32 i = 1; i < list.GetCount(); i++)
      {
        WD_TEST_BOOL(list[i - 1] <= list[i]);
      }
    }

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Various")
  {
    wdDynamicArray<DynamicArrayTestDetail::Dummy> list;
    list.PushBack(1);
    list.PushBack(2);
    list.PushBack(3);
    list.Insert(4, 3);
    list.Insert(0, 1);
    list.Insert(0, 5);

    WD_TEST_BOOL(list[0].a == 1);
    WD_TEST_BOOL(list[1].a == 0);
    WD_TEST_BOOL(list[2].a == 2);
    WD_TEST_BOOL(list[3].a == 3);
    WD_TEST_BOOL(list[4].a == 4);
    WD_TEST_BOOL(list[5].a == 0);
    WD_TEST_BOOL(list.GetCount() == 6);

    list.RemoveAtAndCopy(3);
    list.RemoveAtAndSwap(2);

    WD_TEST_BOOL(list[0].a == 1);
    WD_TEST_BOOL(list[1].a == 0);
    WD_TEST_BOOL(list[2].a == 0);
    WD_TEST_BOOL(list[3].a == 4);
    WD_TEST_BOOL(list.GetCount() == 4);
    WD_TEST_BOOL(list.IndexOf(0) == 1);
    WD_TEST_BOOL(list.LastIndexOf(0) == 2);

    list.PushBack(5);
    WD_TEST_BOOL(list[4].a == 5);
    DynamicArrayTestDetail::Dummy d = list.PeekBack();
    list.PopBack();
    WD_TEST_BOOL(d.a == 5);
    WD_TEST_BOOL(list.GetCount() == 4);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Assignment")
  {
    wdDynamicArray<DynamicArrayTestDetail::Dummy> list;
    for (int i = 0; i < 16; i++)
    {
      list.PushBack(DynamicArrayTestDetail::Dummy(rand()));
    }

    wdDynamicArray<DynamicArrayTestDetail::Dummy> list2;
    for (int i = 0; i < 8; i++)
    {
      list2.PushBack(DynamicArrayTestDetail::Dummy(rand()));
    }

    list = list2;
    WD_TEST_BOOL(list.GetCount() == list2.GetCount());

    list2.Clear();
    WD_TEST_BOOL(list2.GetCount() == 0);

    list2 = list;
    WD_TEST_BOOL(list.PeekBack() == list2.PeekBack());
    WD_TEST_BOOL(list == list2);

    for (int i = 0; i < 16; i++)
    {
      list2.PushBack(DynamicArrayTestDetail::Dummy(rand()));
    }

    list = list2;
    WD_TEST_BOOL(list.PeekBack() == list2.PeekBack());
    WD_TEST_BOOL(list == list2);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Count")
  {
    wdDynamicArray<DynamicArrayTestDetail::Dummy> list;
    for (int i = 0; i < 16; i++)
    {
      list.PushBack(DynamicArrayTestDetail::Dummy(rand()));
    }
    list.SetCount(32);
    list.SetCount(16);

    list.Compact();
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Reserve")
  {
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

    wdDynamicArray<DynamicArrayTestDetail::st> a;

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

    a.Reserve(100);

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

    a.SetCount(10);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(10, 0));

    a.Reserve(100);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 0));

    a.SetCount(100);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(90, 0));

    a.Reserve(200);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(100, 100)); // had to copy some elements over

    a.SetCount(200);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(100, 0));
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Compact")
  {
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

    wdDynamicArray<DynamicArrayTestDetail::st> a;

    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasAllDestructed());

    a.SetCount(100);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(100, 0));

    a.SetCount(200);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(200, 100));

    a.SetCount(10);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 190));

    // no reallocations and copying, if the memory is already available
    a.SetCount(200);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(190, 0));

    a.SetCount(10);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 190));

    // now we remove the spare memory
    a.Compact();
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(10, 10));

    // this time the array needs to be relocated, and thus the already present elements need to be copied
    a.SetCount(200);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(200, 10));

    // this does not deallocate memory
    a.Clear();
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 200));

    a.SetCount(100);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(100, 0));

    // therefore no object relocation
    a.SetCount(200);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(100, 0));

    a.Clear();
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(0, 200));

    // this will deallocate ALL memory
    WD_TEST_BOOL(a.GetHeapMemoryUsage() > 0);
    a.Compact();
    WD_TEST_BOOL(a.GetHeapMemoryUsage() == 0);

    a.SetCount(100);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(100, 0));

    // this time objects need to be relocated
    a.SetCount(200);
    WD_TEST_BOOL(DynamicArrayTestDetail::st::HasDone(200, 100));
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "STL Iterator")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 1000; ++i)
      a1.PushBack(1000 - i - 1);

    // STL sort
    std::sort(begin(a1), end(a1));

    for (wdInt32 i = 1; i < 1000; ++i)
    {
      WD_TEST_BOOL(a1[i - 1] <= a1[i]);
    }

    // foreach
    wdInt32 prev = 0;
    wdInt32 sum1 = 0;
    for (wdInt32 val : a1)
    {
      WD_TEST_BOOL(prev <= val);
      prev = val;
      sum1 += val;
    }

    prev = 1000;
    const auto endIt = rend(a1);
    wdInt32 sum2 = 0;
    for (auto it = rbegin(a1); it != endIt; ++it)
    {
      WD_TEST_BOOL(prev > (*it));
      prev = (*it);
      sum2 += (*it);
    }

    WD_TEST_BOOL(sum1 == sum2);

    // const array
    const wdDynamicArray<wdInt32>& a2 = a1;

    // STL lower bound
    auto lb = std::lower_bound(begin(a2), end(a2), 400);
    WD_TEST_BOOL(*lb == a2[400]);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "STL Reverse Iterator")
  {
    wdDynamicArray<wdInt32> a1;

    for (wdInt32 i = 0; i < 1000; ++i)
      a1.PushBack(1000 - i - 1);

    // STL sort
    std::sort(rbegin(a1), rend(a1));

    for (wdInt32 i = 1; i < 1000; ++i)
    {
      WD_TEST_BOOL(a1[i - 1] >= a1[i]);
    }

    // foreach
    wdUInt32 prev = 1000;
    for (wdUInt32 val : a1)
    {
      WD_TEST_BOOL(prev >= val);
      prev = val;
    }

    // const array
    const wdDynamicArray<wdInt32>& a2 = a1;

    // STL lower bound
    auto lb = std::lower_bound(rbegin(a2), rend(a2), 400);
    WD_TEST_BOOL(*lb == a2[1000 - 400 - 1]);
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "GetArrayPtr")
  {
    wdDynamicArray<wdInt32> a1;
    a1.SetCountUninitialized(10);

    WD_TEST_BOOL(a1.GetArrayPtr().GetCount() == 10);
    WD_TEST_BOOL(a1.GetArrayPtr().GetPtr() == a1.GetData());

    const wdDynamicArray<wdInt32>& a1ref = a1;

    WD_TEST_BOOL(a1ref.GetArrayPtr().GetCount() == 10);
    WD_TEST_BOOL(a1ref.GetArrayPtr().GetPtr() == a1ref.GetData());
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "Swap")
  {
    wdDynamicArray<wdInt32> a1, a2;

    wdInt32 content1[] = {1, 2, 3, 4};
    wdInt32 content2[] = {5, 6, 7, 8, 9};

    a1 = wdMakeArrayPtr(content1);
    a2 = wdMakeArrayPtr(content2);

    wdInt32* a1Ptr = a1.GetData();
    wdInt32* a2Ptr = a2.GetData();

    a1.Swap(a2);

    // The pointers should be simply swapped
    WD_TEST_BOOL(a2Ptr == a1.GetData());
    WD_TEST_BOOL(a1Ptr == a2.GetData());

    // The data should be swapped
    WD_TEST_BOOL(a1.GetArrayPtr() == wdMakeArrayPtr(content2));
    WD_TEST_BOOL(a2.GetArrayPtr() == wdMakeArrayPtr(content1));
  }

#if WD_ENABLED(WD_PLATFORM_64BIT)

  // disabled, because this is a very slow test
  WD_TEST_BLOCK(wdTestBlock::DisabledNoWarning, "Large Allocation")
  {
    const wdUInt32 uiMaxNumElements = 0xFFFFFFFF - 16; // max supported elements due to alignment restrictions

    // this will allocate about 16 GB memory, the pure allocation is really fast
    wdDynamicArray<wdUInt32> byteArray;
    byteArray.SetCountUninitialized(uiMaxNumElements);

    const wdUInt32 uiCheckElements = byteArray.GetCount();
    const wdUInt32 uiSkipElements = 1024;

    // this will touch the memory and thus enforce that it is indeed made available by the OS
    // this takes a while
    for (wdUInt64 i = 0; i < uiCheckElements; i += uiSkipElements)
    {
      const wdUInt32 idx = i & 0xFFFFFFFF;
      byteArray[idx] = idx;
    }

    // check that the assigned values are all correct
    // again, this takes quite a while
    for (wdUInt64 i = 0; i < uiCheckElements; i += uiSkipElements)
    {
      const wdUInt32 idx = i & 0xFFFFFFFF;
      WD_TEST_INT(byteArray[idx], idx);
    }
  }
#endif


  const wdUInt32 uiNumSortItems = 1'000'000;

  struct Item
  {
    bool operator<(const Item& rhs) const { return m_iKey < rhs.m_iKey; }

    wdInt32 m_iKey = 0;
    wdInt32 m_iIndex = 0;
  };

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SortLargeArray (wd-sort)")
  {
    wdDynamicArray<Item> list;
    list.Reserve(uiNumSortItems);

    for (wdUInt32 i = 0; i < uiNumSortItems; i++)
    {
      auto& item = list.ExpandAndGetRef();
      item.m_iIndex = i;
      item.m_iKey = std::rand();
    }

    wdStopwatch sw;
    list.Sort();

    wdTime t = sw.GetRunningTotal();
    wdStringBuilder s;
    s.Format("wd-sort (random keys): {}", t);
    wdTestFramework::Output(wdTestOutput::Details, s);

    for (wdUInt32 i = 1; i < list.GetCount(); i++)
    {
      WD_TEST_BOOL(list[i - 1].m_iKey <= list[i].m_iKey);
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SortLargeArray (std::sort)")
  {
    wdDynamicArray<Item> list;
    list.Reserve(uiNumSortItems);

    for (wdUInt32 i = 0; i < uiNumSortItems; i++)
    {
      auto& item = list.ExpandAndGetRef();
      item.m_iIndex = i;
      item.m_iKey = std::rand();
    }

    wdStopwatch sw;
    std::sort(begin(list), end(list));

    wdTime t = sw.GetRunningTotal();
    wdStringBuilder s;
    s.Format("std::sort (random keys): {}", t);
    wdTestFramework::Output(wdTestOutput::Details, s);

    for (wdUInt32 i = 1; i < list.GetCount(); i++)
    {
      WD_TEST_BOOL(list[i - 1].m_iKey <= list[i].m_iKey);
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SortLargeArray (equal keys) (wd-sort)")
  {
    wdDynamicArray<Item> list;
    list.Reserve(uiNumSortItems);

    for (wdUInt32 i = 0; i < uiNumSortItems; i++)
    {
      auto& item = list.ExpandAndGetRef();
      item.m_iIndex = i;
      item.m_iKey = 42;
    }

    wdStopwatch sw;
    list.Sort();

    wdTime t = sw.GetRunningTotal();
    wdStringBuilder s;
    s.Format("wd-sort (equal keys): {}", t);
    wdTestFramework::Output(wdTestOutput::Details, s);

    for (wdUInt32 i = 1; i < list.GetCount(); i++)
    {
      WD_TEST_BOOL(list[i - 1].m_iKey <= list[i].m_iKey);
    }
  }

  WD_TEST_BLOCK(wdTestBlock::Enabled, "SortLargeArray (equal keys) (std::sort)")
  {
    wdDynamicArray<Item> list;
    list.Reserve(uiNumSortItems);

    for (wdUInt32 i = 0; i < uiNumSortItems; i++)
    {
      auto& item = list.ExpandAndGetRef();
      item.m_iIndex = i;
      item.m_iKey = 42;
    }

    wdStopwatch sw;
    std::sort(begin(list), end(list));

    wdTime t = sw.GetRunningTotal();
    wdStringBuilder s;
    s.Format("std::sort (equal keys): {}", t);
    wdTestFramework::Output(wdTestOutput::Details, s);

    for (wdUInt32 i = 1; i < list.GetCount(); i++)
    {
      WD_TEST_BOOL(list[i - 1].m_iKey <= list[i].m_iKey);
    }
  }
}
