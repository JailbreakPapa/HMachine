...
protected:
  ...
  const wdRTTI* m_pResolvedType = nullptr;
...
protected:
  ...
  virtual void DoPrepareToDie() override;
  void UpdateTypeListSelection(wdVariantType::Enum type);
  void ChangeVariantType(wdVariantType::Enum type);

  virtual wdResult GetVariantTypeDisplayName(wdVariantType::Enum type, wdStringBuilder& out_sName) const;
