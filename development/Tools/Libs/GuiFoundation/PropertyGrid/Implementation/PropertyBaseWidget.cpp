...
wdQtPropertyWidget::~wdQtPropertyWidget() = default;
...
wdQtPropertyTypeWidget::~wdQtPropertyTypeWidget() = default;
...
wdQtPropertyStandardTypeContainerWidget::~wdQtPropertyStandardTypeContainerWidget() = default;
...
wdQtVariantPropertyWidget::~wdQtVariantPropertyWidget() = default;

void wdQtVariantPropertyWidget::OnInit()
{
  wdStringBuilder sName;
  for (int i = wdVariantType::Invalid; i < wdVariantType::LastExtendedType; ++i)
  {
    auto type = static_cast<wdVariantType::Enum>(i);
    if (GetVariantTypeDisplayName(type, sName).Succeeded())
    {
      m_pTypeList->addItem(wdTranslate(sName), i);
    }
  }

  connect(m_pTypeList, &QComboBox::currentIndexChanged,
    [this](int index) {
      ChangeVariantType(static_cast<wdVariantType::Enum>(m_pTypeList->itemData(index).toInt()));
    });
}
...
void wdQtVariantPropertyWidget::InternalSetValue(const wdVariant& value)
{
  ...
  if (pNewtSubType != m_pCurrentSubType || m_pWidget == nullptr)
  {
    ...
    {
      m_pWidget = new wdQtUnsupportedPropertyWidget("Multi-selection has varying types");
    }
    else
    {
      m_pWidget = new wdQtUnsupportedPropertyWidget("<Invalid>");
    }
    m_pWidget->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Preferred);
...
void wdQtVariantPropertyWidget::UpdateTypeListSelection(wdVariantType::Enum type)
{
  wdQtScopedBlockSignals bs(m_pTypeList);
...
wdResult wdQtVariantPropertyWidget::GetVariantTypeDisplayName(wdVariantType::Enum type, wdStringBuilder& out_sName) const
{
  if (type == wdVariantType::FirstStandardType || type == wdVariantType::StringView || type == wdVariantType::DataBuffer || type >= wdVariantType::LastStandardType)
    return WD_FAILURE;

  const wdRTTI* pVariantEnum = wdGetStaticRTTI<wdVariantType>();
  if (wdReflectionUtils::EnumerationToString(pVariantEnum, type, out_sName) == false)
    return WD_FAILURE;
