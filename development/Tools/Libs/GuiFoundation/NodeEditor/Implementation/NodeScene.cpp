...
void wdQtNodeScene::SetDocumentNodeManager(const wdDocumentNodeManager* pManager)
{
  ...
  if (m_pManager != nullptr)
  {
    m_pManager->m_NodeEvents.AddEventHandler(wdMakeDelegate(&wdQtNodeScene::NodeEventsHandler, this));
...
void wdQtNodeScene::RecreateQtPins(const wdDocumentObject* pObject)
{
  wdQtNode* pNode = m_Nodes[pObject];
  pNode->CreatePins();
  pNode->UpdateState();
...
void wdQtNodeScene::NodeEventsHandler(const wdDocumentNodeManagerEvent& e)
{
  switch (e.m_EventType)
  {
    ...
    case wdDocumentNodeManagerEvent::Type::BeforePinsChanged:
      break;

    case wdDocumentNodeManagerEvent::Type::AfterPinsChanged:
