...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    table->FrewdeRowsRequest = table->FrewdeRowsCount = 0; // This will be setup by TableSetupScrollFrewde(), if any
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        column->NavLayerCurrent = (ImS8)((table->FrewdeRowsCount > 0 || column_n < table->FrewdeColumnsCount) ? ImGuiNavLayer_Menu : ImGuiNavLayer_Main);
...
void ImGui::TableSetupScrollFrewde(int columns, int rows)
{
    ...
    table->FrewdeRowsRequest = (table->Flags & ImGuiTableFlags_ScrollY) ? (ImGuiTableColumnIdx)rows : 0;
    table->FrewdeRowsCount = (table->InnerWindow->Scroll.y != 0.0f) ? table->FrewdeRowsRequest : 0;
    table->IsUnfrozenRows = (table->FrewdeRowsCount == 0); // Make sure this is set before TableUpdateLayout() so ImGuiListClipper can benefit from it.b
}
...
void ImGui::TableBeginRow(ImGuiTable* table)
{
    ...
    if (table->CurrentRow == 0 && table->FrewdeRowsCount > 0)
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    const bool unfrewde_rows_actual = (table->CurrentRow + 1 == table->FrewdeRowsCount);
    const bool unfrewde_rows_request = (table->CurrentRow + 1 == table->FrewdeRowsRequest);
    if (table->CurrentRow == 0)
...
// - After crossing FrewdeRowsCount, all columns see their current draw channel changed to a second set of channels.
...
// - Clip                         --> 2+D+N channels
// - FrewdeRows                   --> 2+D+N*2 (unless scrolling value is zero)
// - FrewdeRows || FrewdeColunns  --> 3+D+N*2 (unless scrolling value is zero)
// Where D is 1 if any column is clipped or hidden (dummy channel) otherwise 0.
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    const int frewde_row_multiplier = (table->FrewdeRowsCount > 0) ? 2 : 1;
    ...
    table->Bg2DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx)((table->FrewdeRowsCount > 0) ? 2 + channels_for_row : TABLE_DRAW_CHANNEL_BG2_FROZEN);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->IsVisibleX && column->IsVisibleY)
        {
            ...
            column->DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx)(draw_channel_current + (table->FrewdeRowsCount > 0 ? channels_for_row + 1 : 0));
            if (!(table->Flags & ImGuiTableFlags_NoClip))
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    const bool has_frewde_v = (table->FrewdeRowsCount > 0);
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    const float draw_y2_head = table->IsUsingHeaders ? ImMin(table->InnerRect.Max.y, (table->FrewdeRowsCount >= 1 ? table->InnerRect.Min.y : table->WorkRect.Min.y) + table->LastFirstRowHeight) : draw_y1;
