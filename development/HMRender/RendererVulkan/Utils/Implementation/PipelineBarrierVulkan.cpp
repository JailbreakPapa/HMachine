...
namespace
{
  vk::ImageSubresourceRange CreateSubRange(const vk::ImageSubresourceRange& fullRange, wdUInt32 uiLayer, wdUInt32 uiMipLevel)
  {
...
void wdPipelineBarrierVulkan::SetCommandBuffer(vk::CommandBuffer* pCommandBuffer)
{
  m_pCommandBuffer = pCommandBuffer;
}

void wdPipelineBarrierVulkan::Flush()
{
  if (m_srcStageMask || m_dstStageMask)
  {
    ...
    {
      wdLog::Warning("Layout Changed {}: {} -> {}", wdArgP(img.image), vk::to_string(img.oldLayout).c_str(), vk::to_string(img.newLayout).c_str());
    }
#endif
...
void wdPipelineBarrierVulkan::Submit()
{
  Flush();
  ...
  for (auto it = m_imageState.GetIterator(); it.IsValid();)
  {
    ...
    const wdUInt32 uiCount = it.Value().m_subElementLayout.GetCount();
...
bool wdPipelineBarrierVulkan::IsDirty(vk::Image image, const vk::ImageSubresourceRange& subResources) const
{
...
bool wdPipelineBarrierVulkan::IsDirty() const
{
  return m_srcStageMask || m_dstStageMask;
}

bool wdPipelineBarrierVulkan::IsDirty(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::AccessFlags dstAccess)
{
...
void wdPipelineBarrierVulkan::AccessBuffer(const wdGALBufferVulkan* pBuffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
  SubBufferState subState;
  subState.m_offset = offset;
  subState.m_length = length;
...
void wdPipelineBarrierVulkan::EnsureImageLayout(const wdGALTextureVulkan* pTexture, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  EnsureImageLayout(pTexture, pTexture->GetFullRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void wdPipelineBarrierVulkan::EnsureImageLayout(const wdGALRenderTargetViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const wdGALTextureVulkan*>(pTextureView->GetTexture()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void wdPipelineBarrierVulkan::EnsureImageLayout(const wdGALResourceViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const wdGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void wdPipelineBarrierVulkan::EnsureImageLayout(const wdGALUnorderedAccessViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const wdGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void wdPipelineBarrierVulkan::EnsureImageLayout(const wdGALTextureVulkan* pTexture, vk::ImageSubresourceRange subResources, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  const vk::ImageSubresourceRange fullRange = pTexture->GetFullRange();
  if (wdGALResourceFormat::IsStencilFormat(pTexture->GetDescription().m_Format))
  {
  ...
  if (pState)
  {
    ...
    if (bIsFullRange)
    {
      ...
      else
      {
        ...
        const wdUInt32 uiLastLayer = subResources.baseArrayLayer + subResources.layerCount;
        const wdUInt32 uiLastLevel = subResources.baseMipLevel + subResources.levelCount;
        for (wdUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
        {
          for (wdUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
          {
            const wdUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * fullRange.levelCount;
...
void wdPipelineBarrierVulkan::SetInitialImageState(const wdGALTextureVulkan* pTexture, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
  auto it = m_imageState.Find(pTexture->GetImage());
  WD_ASSERT_DEBUG(!it.IsValid(), "Can't set initial state, texture is already tracked.");
...
void wdPipelineBarrierVulkan::TextureDestroyed(const wdGALTextureVulkan* pTexture)
{
  auto it = m_imageState.Find(pTexture->GetImage());
  if (it.IsValid())
  {
    ...
    if (it.Value().m_dirty.IsAnyBitSet())
    {
      ...
      for (wdInt32 i = ((wdInt32)m_imageBarriers.GetCount()) - 1; i >= 0; --i)
      {
...
void wdPipelineBarrierVulkan::BufferDestroyed(const wdGALBufferVulkan* pBuffer)
{
...
void wdPipelineBarrierVulkan::FullBarrier()
{
  vk::MemoryBarrier memoryBarrier;
...
bool wdPipelineBarrierVulkan::AddBufferBarrierInternal(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
...
bool wdPipelineBarrierVulkan::IsDirtyInternal(const BufferState& state, const SubBufferState& subState) const
{
  const wdUInt32 uiCount = state.m_subBufferState.GetCount();
  for (wdUInt32 i = 0; i < uiCount; i++)
  {
...
bool wdPipelineBarrierVulkan::AddImageBarrierInternal(vk::Image image, const vk::ImageSubresourceRange& subResources, vk::ImageLayout srcLayout, vk::AccessFlags srcAccess, vk::ImageLayout dstLayout, vk::AccessFlags dstAccess, bool bDiscardSource)
{
...
bool wdPipelineBarrierVulkan::IsDirtyInternal(const ImageState& state, const vk::ImageSubresourceRange& subResources) const
{
  ...
  {
    wdUInt32 uiLevelCount = state.m_pTexture->GetDescription().m_uiMipLevelCount;
    ...
    for (wdUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
    {
      for (wdUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
      {
        const wdUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * uiLevelCount;
