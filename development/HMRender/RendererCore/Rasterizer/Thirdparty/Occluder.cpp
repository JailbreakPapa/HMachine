...
Occluder::~Occluder()
{
  WD_DELETE_RAW_BUFFER(wdFoundation::GetAlignedAllocator(), m_vertexData);
...
void Occluder::bake(const __m128* vertices, wdUInt32 numVertices, __m128 refMin, __m128 refMax)
{
  assert(numVertices % 16 == 0);

  // Simple k-means clustering by normal direction to improve backface culling efficiency
  wdHybridArray<__m128, 32, wdAlignedAllocatorWrapper> quadNormals;
  for (wdUInt32 i = 0; i < numVertices; i += 4)
  {
  ...
  wdHybridArray<__m128, 32, wdAlignedAllocatorWrapper> centroids;
  wdHybridArray<uint32_t, 32> centroidAssignment;
  ...
  wdHybridArray<__m128, 64, wdAlignedAllocatorWrapper> orderedVertices;
  for (uint32_t k = 0; k < centroids.GetCount(); ++k)
  {
    for (wdUInt32 j = 0; j < numVertices / 4; ++j)
    {
  ...
  occluder->m_vertexData = WD_NEW_RAW_BUFFER(wdFoundation::GetAlignedAllocator(), __m256i, orderedVertices.GetCount() * 4);

  for (wdUInt32 i = 0; i < orderedVertices.GetCount(); i += 32)
  {
    __m128i v[8];

    for (wdUInt32 j = 0; j < 4; ++j)
    {
  ...
  for (wdUInt32 i = 0; i < orderedVertices.GetCount(); ++i)
  {
...
WD_STATICLINK_FILE(RendererCore, RendererCore_Rasterizer_Thirdparty_Occluder);
