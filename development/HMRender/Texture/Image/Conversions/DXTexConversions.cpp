...
wdCVarBool cvar_TexturePenalizeDXConversions("Texture.PenalizeDXConversions", false, wdCVarFlags::RequiresRestart, "Add a penalty to DirectX-based conversion when choosing how to convert textures");

#if WD_ENABLED(WD_PLATFORM_WINDOWS_DESKTOP)
#  define WD_SUPPORTS_DIRECTXTEX WD_ON
#else
#  define WD_SUPPORTS_DIRECTXTEX WD_OFF
#endif

#if WD_ENABLED(WD_SUPPORTS_DIRECTXTEX)
...
#  endif
    ...
    if (SUCCEEDED(hr))
    {
      ...
      if (SUCCEEDED(hr))
      {
        wdStringUtf8 sDesc(desc.Description);
        wdLog::Dev("Using DirectCompute on \"{0}\"", sDesc.GetData());
      }
  ...
  public:
    ...
    wdArrayPtr<const wdImageConversionEntry> getConvertors() const { return m_SupportedConversions; }
    ...
    void Init()
    {
      ...
      for (auto& entry : wdArrayPtr<wdImageConversionEntry>(s_sourceConversions))
      {
  ...
  private:
    ...
    wdArrayPtr<const wdImageConversionEntry> m_SupportedConversions;

    constexpr static int s_numConversions = 5;
    static wdImageConversionEntry s_sourceConversions[s_numConversions];
  ...
  wdImageConversionEntry DeviceAndConversionTable::s_sourceConversions[s_numConversions] = {
...
// clang-format off
WD_BEGIN_SUBSYSTEM_DECLARATION(TexConv, DXTexConversions)
...
class wdImageConversion_CompressDxTex : public wdImageConversionStepCompressBlocks
{
public:
  ...
  {
    const wdUInt32 targetWidth = uiNumBlocksX * wdImageFormat::GetBlockWidth(targetFormat);
    const wdUInt32 targetHeight = uiNumBlocksY * wdImageFormat::GetBlockHeight(targetFormat);

    Image srcImg;
    srcImg.width = targetWidth;
    srcImg.height = targetHeight;
    srcImg.rowPitch = static_cast<size_t>(wdImageFormat::GetRowPitch(sourceFormat, targetWidth));
    srcImg.slicePitch = static_cast<size_t>(wdImageFormat::GetDepthPitch(sourceFormat, targetWidth, targetHeight));
    ...
    srcImg.format = (DXGI_FORMAT)wdImageFormatMappings::ToDxgiFormat(wdImageFormat::AsLinear(sourceFormat));
    ...
    const DXGI_FORMAT dxgiTargetFormat = (DXGI_FORMAT)wdImageFormatMappings::ToDxgiFormat(wdImageFormat::AsLinear(targetFormat));
    ...
    target.CopyFrom(wdConstByteBlobPtr(dxDstImage.GetPixels(), static_cast<wdUInt32>(dxDstImage.GetPixelsSize())));

    return WD_SUCCESS;
  }
};

static wdImageConversion_CompressDxTex s_conversion_compressDxTex;

#endif



WD_STATICLINK_FILE(Texture, Texture_Image_Conversions_DXTexConversions);
