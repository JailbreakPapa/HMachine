...
///
/// Derive from this class to implement custom component types. Also add the WD_DECLARE_COMPONENT_TYPE macro to your class declaration.
/// Also add a WD_BEGIN_COMPONENT_TYPE/WD_END_COMPONENT_TYPE block to a cpp file. In that block you can add reflected members or message
/// handlers. Note that every component type needs a corresponding manager type. Take a look at wdComponentManagerSimple for a simple
...
/// the corresponding manager. Never store a direct pointer to a component but store an wdComponentHandle instead.
class WD_CORE_DLL wdComponent : public wdReflectedClass
{
  WD_ADD_DYNAMIC_REFLECTION(wdComponent, wdReflectedClass);
...
public:
  ...
  ///
  /// \sa wdGameObject::IsActive(), wdGameObject::SetActiveFlag()
  ...
  wdComponentManagerBase* GetOwningManager();

  /// \brief Returns the corresponding manager for this component.
  const wdComponentManagerBase* GetOwningManager() const;

  /// \brief Returns the owner game object if the component is attached to one or nullptr.
  wdGameObject* GetOwner();

  /// \brief Returns the owner game object if the component is attached to one or nullptr.
  const wdGameObject* GetOwner() const;
  ...
  virtual void DeserializeComponent(wdWorldReader& inout_stream);
  ...
  WD_ALWAYS_INLINE bool SendMessage(wdMessage& ref_msg) { return SendMessageInternal(ref_msg, false); }
  WD_ALWAYS_INLINE bool SendMessage(wdMessage& ref_msg) const { return SendMessageInternal(ref_msg, false); }

  /// \brief Queues the message for the given phase. The message is processed after the given delay in the corresponding phase.
  void PostMessage(const wdMessage& msg, wdTime delay = wdTime::Zero(), wdObjectMsgQueueType::Enum queueType = wdObjectMsgQueueType::NextFrame) const;

  /// \brief Returns whether the given Message is handled by this component.
  virtual bool HandlesMessage(const wdMessage& msg) const;

  /// Be careful to check which flags may already be in use by base classes.
  void SetUserFlag(wdUInt8 uiFlagIndex, bool bSet);

  /// \brief Retrieves a custom flag. Index must be between 0 and 7.
  bool GetUserFlag(wdUInt8 uiFlagIndex) const;

  /// \brief Adds wdObjectFlags::CreatedByPrefab to the component. See the flag for details.
  void SetCreatedByPrefab() { m_ComponentFlags.Add(wdObjectFlags::CreatedByPrefab); }

  /// \brief Checks whether the wdObjectFlags::CreatedByPrefab flag is set on this component.
  bool WasCreatedByPrefab() const { return m_ComponentFlags.IsSet(wdObjectFlags::CreatedByPrefab); }
...
protected:
  ...
  virtual wdWorldModuleTypeId GetTypeId() const = 0;
  virtual wdComponentMode::Enum GetMode() const = 0;
  ...
  /// Therefore the global position has already been computed and the owner wdGameObject is set.
  ...
  /// editor.
  ///
  /// Additionally, OnSimulationStarted() is only ever executed once on a component, even if the wdWorld pauses and resumes world simulation
  ...
  virtual bool OnUnhandledMessage(wdMessage& msg, bool bWasPostedMsg);

  /// \brief When EnableUnhandledMessageHandler() was activated, this is called for all messages for which there is no dedicated message handler.
  ///
  /// \return Should return true if the given message was handled, false otherwise.
  virtual bool OnUnhandledMessage(wdMessage& msg, bool bWasPostedMsg) const;

protected:
  /// Messages will be dispatched to this type. Default is what GetDynamicRTTI() returns, can be redirected if necessary.
  const wdRTTI* m_pMessageDispatchType = nullptr;
...
private:
  ...
  wdGameObject* Reflection_GetOwner() const;
  wdWorld* Reflection_GetWorld() const;
  ...
  wdComponentId m_InternalId;
  wdBitflags<wdObjectFlags> m_ComponentFlags = wdObjectFlags::ActiveFlag;
  wdUInt32 m_uiUniqueID = wdInvalidIndex;

  wdComponentManagerBase* m_pManager = nullptr;
  wdGameObject* m_pOwner = nullptr;

  static wdWorldModuleTypeId s_TypeId;
};

struct wdComponent_ScriptBaseClassFunctions
{
  enum Enum
  {
    Initialize,
