...
class wdEventMessageHandlerComponent;
...
class WD_CORE_DLL wdWorld final
{
public:
  /// \brief Creates a new world with the given name.
  wdWorld(wdWorldDesc& ref_desc);
  ~wdWorld();
  ...
  wdGameObjectHandle CreateObject(const wdGameObjectDesc& desc, wdGameObject*& out_pObject);
  ...
  void DeleteObjectNow(const wdGameObjectHandle& hObject, bool bAlsoDeleteEmptyParents = true);
  ...
  ///
  /// If bAlsoDeleteEmptyParents is set, any ancestor object that has no other children and no components, will also get deleted.
  void DeleteObjectDelayed(const wdGameObjectHandle& hObject, bool bAlsoDeleteEmptyParents = true);
  ...
  const wdEvent<const wdGameObject*>& GetObjectDeletionEvent() const;

  /// \brief Returns whether the given handle corresponds to a valid object.
  bool IsValidObject(const wdGameObjectHandle& hObject) const;
  ...
  [[nodiscard]] bool TryGetObject(const wdGameObjectHandle& hObject, wdGameObject*& out_pObject);

  /// \brief Returns whether an object with the given handle exists and if so writes out the corresponding pointer to out_pObject.
  [[nodiscard]] bool TryGetObject(const wdGameObjectHandle& hObject, const wdGameObject*& out_pObject) const;
  ...
  [[nodiscard]] bool TryGetObjectWithGlobalKey(const wdTempHashedString& sGlobalKey, wdGameObject*& out_pObject);

  /// \brief Returns whether an object with the given global key exists and if so writes out the corresponding pointer to out_pObject.
  [[nodiscard]] bool TryGetObjectWithGlobalKey(const wdTempHashedString& sGlobalKey, const wdGameObject*& out_pObject) const;


  /// \brief Returns the total number of objects in this world.
  wdUInt32 GetObjectCount() const;

  /// \brief Returns an iterator over all objects in this world in no specific order.
  wdInternal::WorldData::ObjectIterator GetObjects();

  /// \brief Returns an iterator over all objects in this world in no specific order.
  wdInternal::WorldData::ConstObjectIterator GetObjects() const;
  ...
  /// (false) traversal.
  using VisitorFunc = wdInternal::WorldData::VisitorFunc;
  ...
  /// \brief Creates an instance of the given module type or derived type or returns a pointer to an already existing instance.
  wdWorldModule* GetOrCreateModule(const wdRTTI* pRtti);
  ...
  /// \brief Deletes the module of the given type or derived types.
  void DeleteModule(const wdRTTI* pRtti);
  ...
  /// \brief Returns the instance to the given module type or derived types.
  wdWorldModule* GetModule(const wdRTTI* pRtti);

  /// \brief Returns the instance to the given module type or derived types.
  const wdWorldModule* GetModule(const wdRTTI* pRtti) const;
  ...
  wdComponentManagerBase* GetOrCreateManagerForComponentType(const wdRTTI* pComponentRtti);
  ...
  /// \brief Returns the component manager that handles the given rtti component type.
  wdComponentManagerBase* GetManagerForComponentType(const wdRTTI* pComponentRtti);

  /// \brief Returns the component manager that handles the given rtti component type.
  const wdComponentManagerBase* GetManagerForComponentType(const wdRTTI* pComponentRtti) const;
  ...
  [[nodiscard]] bool TryGetComponent(const wdComponentHandle& hComponent, ComponentType*& out_pComponent);

  /// \brief Returns whether a component with the given handle exists and if so writes out the corresponding pointer to out_pComponent.
  template <typename ComponentType>
  [[nodiscard]] bool TryGetComponent(const wdComponentHandle& hComponent, const ComponentType*& out_pComponent) const;
  ...
  wdComponentInitBatchHandle CreateComponentInitBatch(wdStringView sBatchName, bool bMustFinishWithinOneFrame = true);

  /// \brief Deletes a component init batch. It must be completely processed before it can be deleted.
  void DeleteComponentInitBatch(const wdComponentInitBatchHandle& hBatch);
  ...
  /// given init batch.
  void BeginAddingComponentsToInitBatch(const wdComponentInitBatchHandle& hBatch);

  /// \brief End adding components to the given batch. Components created after this call are added to the default init batch.
  void EndAddingComponentsToInitBatch(const wdComponentInitBatchHandle& hBatch);

  /// \brief After all components have been added to the init batch call submit to start processing the batch.
  void SubmitComponentInitBatch(const wdComponentInitBatchHandle& hBatch);
  ...
  bool IsComponentInitBatchCompleted(const wdComponentInitBatchHandle& hBatch, double* pCompletionFactor = nullptr);
  ...
  void CancelComponentInitBatch(const wdComponentInitBatchHandle& hBatch);
  ...
  void SendMessage(const wdGameObjectHandle& hReceiverObject, wdMessage& ref_msg);

  /// \brief Sends a message to all components of the receiverObject and all its children.
  void SendMessageRecursive(const wdGameObjectHandle& hReceiverObject, wdMessage& ref_msg);
  ...
  void PostMessage(const wdGameObjectHandle& hReceiverObject, const wdMessage& msg, wdTime delay,
    wdObjectMsgQueueType::Enum queueType = wdObjectMsgQueueType::NextFrame) const;
  ...
  void PostMessageRecursive(const wdGameObjectHandle& hReceiverObject, const wdMessage& msg, wdTime delay,
    wdObjectMsgQueueType::Enum queueType = wdObjectMsgQueueType::NextFrame) const;

  /// \brief Sends a message to the component.
  void SendMessage(const wdComponentHandle& hReceiverComponent, wdMessage& ref_msg);
  ...
  void PostMessage(const wdComponentHandle& hReceiverComponent, const wdMessage& msg, wdTime delay,
    wdObjectMsgQueueType::Enum queueType = wdObjectMsgQueueType::NextFrame) const;

  /// \brief Finds the closest (parent) object, starting at pSearchObject, which has an wdComponent that handles the given message and returns all
  /// matching components owned by that object. If a wdEventMessageHandlerComponent is found the search is stopped even if it doesn't handle the given message.
  ///
  /// If no such parent object exists, it searches for all wdEventMessageHandlerComponent instances that are set to 'handle global events'
  /// that handle messages of the given type.
  void FindEventMsgHandlers(const wdMessage& msg, wdGameObject* pSearchObject, wdDynamicArray<wdComponent*>& out_components);

  /// \copydoc wdWorld::FindEventMsgHandlers()
  void FindEventMsgHandlers(const wdMessage& msg, const wdGameObject* pSearchObject, wdDynamicArray<const wdComponent*>& out_components) const;
  ...
  /// the game objects. See wdWorld for a detailed description of the update phases.
  void Update();

  /// \brief Returns a task implementation that calls Update on this world.
  const wdSharedPtr<wdTask>& GetUpdateTask();


  /// \brief Returns the spatial system that is associated with this world.
  wdSpatialSystem* GetSpatialSystem();

  /// \brief Returns the spatial system that is associated with this world.
  const wdSpatialSystem* GetSpatialSystem() const;
  ...
  void GetCoordinateSystem(const wdVec3& vGlobalPosition, wdCoordinateSystem& out_coordinateSystem) const;

  /// \brief Sets the coordinate system provider that should be used in this world.
  void SetCoordinateSystemProvider(const wdSharedPtr<wdCoordinateSystemProvider>& pProvider);

  /// \brief Returns the coordinate system provider that is associated with this world.
  wdCoordinateSystemProvider& GetCoordinateSystemProvider();

  /// \brief Returns the coordinate system provider that is associated with this world.
  const wdCoordinateSystemProvider& GetCoordinateSystemProvider() const;
  ...
  wdRandom& GetRandomNumberGenerator();
  ...
  wdDoubleBufferedStackAllocator* GetStackAllocator();

  /// \brief Mark the world for reading by using WD_LOCK(world.GetReadMarker()). Multiple threads can read simultaneously if none is
  /// writing.
  wdInternal::WorldData::ReadMarker& GetReadMarker() const;

  /// \brief Mark the world for writing by using WD_LOCK(world.GetWriteMarker()). Only one thread can write at a time.
  wdInternal::WorldData::WriteMarker& GetWriteMarker();
  ...
  void SetMaxInitializationTimePerFrame(wdTime maxInitTime);
  ...
  using ReferenceResolver = wdDelegate<wdGameObjectHandle(const void*, wdComponentHandle hThis, const char* szProperty)>;

  /// \brief If set, this delegate can be used to map some data (GUID or string) to an wdGameObjectHandle.
  ///
  ...
  /// \name Helper methods to query wdWorld limits
  ///@{
  static constexpr wdUInt64 GetMaxNumGameObjects();
  static constexpr wdUInt64 GetMaxNumHierarchyLevels();
  static constexpr wdUInt64 GetMaxNumComponentsPerType();
  static constexpr wdUInt64 GetMaxNumWorldModules();
  static constexpr wdUInt64 GetMaxNumComponentTypes();
  static constexpr wdUInt64 GetMaxNumWorlds();
  ///@}

public:
  /// \brief Returns the number of active worlds.
  static wdUInt32 GetWorldCount();

  /// \brief Returns the world with the given index.
  static wdWorld* GetWorld(wdUInt32 uiIndex);

  /// \brief Returns the world for the given game object handle.
  static wdWorld* GetWorld(const wdGameObjectHandle& hObject);

  /// \brief Returns the world for the given component handle.
  static wdWorld* GetWorld(const wdComponentHandle& hComponent);

private:
  friend class wdGameObject;
  friend class wdWorldModule;
  ...
  wdGameObject* GetObjectUnchecked(wdUInt32 uiIndex) const;

  void SetParent(wdGameObject* pObject, wdGameObject* pNewParent,
    wdGameObject::TransformPreservation preserve = wdGameObject::TransformPreservation::PreserveGlobal);
  void LinkToParent(wdGameObject* pObject);
  void UnlinkFromParent(wdGameObject* pObject);

  void SetObjectGlobalKey(wdGameObject* pObject, const wdHashedString& sGlobalKey);
  wdStringView GetObjectGlobalKey(const wdGameObject* pObject) const;

  void PostMessage(const wdGameObjectHandle& receiverObject, const wdMessage& msg, wdObjectMsgQueueType::Enum queueType, wdTime delay, bool bRecursive) const;
  void ProcessQueuedMessage(const wdInternal::WorldData::MessageQueue::Entry& entry);
  void ProcessQueuedMessages(wdObjectMsgQueueType::Enum queueType);

  template <typename World, typename GameObject, typename Component>
  static void FindEventMsgHandlers(World& world, const wdMessage& msg, GameObject pSearchObject, wdDynamicArray<Component>& out_components);

  void RegisterUpdateFunction(const wdWorldModule::UpdateFunctionDesc& desc);
  void DeregisterUpdateFunction(const wdWorldModule::UpdateFunctionDesc& desc);
  void DeregisterUpdateFunctions(wdWorldModule* pModule);

  /// \brief Used by component managers to queue a new component for initialization during the next update
  void AddComponentToInitialize(wdComponentHandle hComponent);

  void UpdateFromThread();
  void UpdateSynchronous(const wdArrayPtr<wdInternal::WorldData::RegisteredUpdateFunction>& updateFunctions);
  void UpdateAsynchronous();

  // returns if the batch was completely initialized
  bool ProcessInitializationBatch(wdInternal::WorldData::InitBatch& batch, wdTime endTime);
  void ProcessComponentsToInitialize();
  void ProcessUpdateFunctionsToRegister();
  wdResult RegisterUpdateFunctionInternal(const wdWorldModule::UpdateFunctionDesc& desc);

  void DeleteDeadObjects();
  void DeleteDeadComponents();

  void PatchHierarchyData(wdGameObject* pObject, wdGameObject::TransformPreservation preserve);
  void RecreateHierarchyData(wdGameObject* pObject, bool bWasDynamic);

  bool ReportErrorWhenStaticObjectMoves() const;

  wdSharedPtr<wdTask> m_pUpdateTask;

  wdInternal::WorldData m_Data;

  using QueuedMsgMetaData = wdInternal::WorldData::QueuedMsgMetaData;

  wdUInt32 m_uiIndex;
  static wdStaticArray<wdWorld*, WD_MAX_WORLDS> s_Worlds;
};

WD_DECLARE_REFLECTABLE_TYPE(WD_CORE_DLL, wdWorld);
