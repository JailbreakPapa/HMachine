...
/// It ALSO allows to construct the wdLuaWrapper with a working lua_State-Pointer and thus
...
///
/// \note Lua starts counting at 1, not at 0. However wdLuaWrapper does NOT do this, but uses the C++ convention instead!
...
class WD_CORE_DLL wdLuaWrapper
{
  WD_DISALLOW_COPY_AND_ASSIGN(wdLuaWrapper);
...
public:
  ...
  ///   log.
  wdResult ExecuteString(const char* szString, const char* szDebugChunkName = "chunk",
    wdLogInterface* pLogInterface = nullptr) const; // [tested]
  ...
  /// The table to open needs to be in scope itself. Returns WD_FAILURE, if it's not possible (the table does not exist in this scope).
  wdResult OpenTable(const char* szTable); // [tested]

  /// Opens the Table n, that was passed to a C-Function on its Parameter-Stack.
  wdResult OpenTableFromParameter(wdUInt32 uiFunctionParameter); // [tested]
  ...
  int GetIntVariable(const char* szName, wdInt32 iDefault = 0) const; // [tested]
  ...
  void SetVariable(const char* szName, wdInt32 iValue) const; // [tested]
  ...
  /// Sets the Variable with the given name (in scope) with the given value.
  void SetVariable(const char* szName, const char* szValue, wdUInt32 uiLen) const; // [tested]
  ...
  ///
  /// Returns WD_FAILURE if anything went wrong during function execution. Reports errors via \a pLogInterface.
  wdResult CallPreparedFunction(wdUInt32 uiExpectedReturnValues = 0, wdLogInterface* pLogInterface = nullptr); // [tested]
  ...
  /// {
  ///   wdLuaWrapper s(state);
  ///   .. do something ..
  ///   return s.ReturnToScript();
  /// }
  /// \endcode
  wdInt32 ReturnToScript() const; // [tested]
  ...
  void PushParameter(wdInt32 iParam); // [tested]
  ...
  /// Pushes a parameter on the stack to be passed to the next function called.
  /// Do this after PrepareFunctionCall() and before CallPreparedFunction().
  void PushParameter(const char* szParam, wdUInt32 uiLength); // [tested]
  ...
  wdUInt32 GetNumberOfFunctionParameters() const; // [tested]

  /// Checks the nth Parameter passed to a C-Function for its type.
  bool IsParameterInt(wdUInt32 uiParameter) const; // [tested]

  /// Checks the nth Parameter passed to a C-Function for its type.
  bool IsParameterBool(wdUInt32 uiParameter) const; // [tested]

  /// Checks the nth Parameter passed to a C-Function for its type.
  bool IsParameterFloat(wdUInt32 uiParameter) const; // [tested]

  /// Checks the nth Parameter passed to a C-Function for its type.
  bool IsParameterTable(wdUInt32 uiParameter) const; // [tested]

  /// Checks the nth Parameter passed to a C-Function for its type.
  bool IsParameterString(wdUInt32 uiParameter) const; // [tested]

  /// Checks the nth Parameter passed to a C-Function for its type.
  bool IsParameterNil(wdUInt32 uiParameter) const; // [tested]

  /// Returns the Value of the nth Parameter.
  int GetIntParameter(wdUInt32 uiParameter) const; // [tested]

  /// Returns the Value of the nth Parameter.
  bool GetBoolParameter(wdUInt32 uiParameter) const; // [tested]

  /// Returns the Value of the nth Parameter.
  float GetFloatParameter(wdUInt32 uiParameter) const; // [tested]

  /// Returns the Value of the nth Parameter.
  const char* GetStringParameter(wdUInt32 uiParameter) const; // [tested]
  ...
  void PushReturnValue(wdInt32 iParam); // [tested]
  ...
  /// Pushes a value as a return value for a called C-Function
  void PushReturnValue(const char* szParam, wdUInt32 uiLength); // [tested]
  ...
  bool IsReturnValueInt(wdUInt32 uiReturnValue) const; // [tested]

  /// Checks the nth return-value passed to a C-Function for its type.
  bool IsReturnValueBool(wdUInt32 uiReturnValue) const; // [tested]

  /// Checks the nth return-value passed to a C-Function for its type.
  bool IsReturnValueFloat(wdUInt32 uiReturnValue) const; // [tested]

  /// Checks the nth return-value passed to a C-Function for its type.
  bool IsReturnValueString(wdUInt32 uiReturnValue) const; // [tested]

  /// Checks the nth return-value passed to a C-Function for its type.
  bool IsReturnValueNil(wdUInt32 uiReturnValue) const; // [tested]


  /// Returns the value of the nth return-value.
  int GetIntReturnValue(wdUInt32 uiReturnValue) const; // [tested]

  /// Returns the value of the nth return-value.
  bool GetBoolReturnValue(wdUInt32 uiReturnValue) const; // [tested]

  /// Returns the value of the nth return-value.
  float GetFloatReturnValue(wdUInt32 uiReturnValue) const; // [tested]

  /// Returns the value of the nth return-value.
  const char* GetStringReturnValue(wdUInt32 uiReturnValue) const; // [tested]
...
private:
  ...
  struct wdScriptStates
  {
    ...
    wdInt32 m_iParametersPushed = 0;

    /// How many Tables have been opened inside the Lua-Script.
    wdInt32 m_iOpenTables = 0;

    /// How many values the called Lua-function should return
    wdInt32 m_iLuaReturnValues = 0;
  };

  wdScriptStates m_States;

  static const wdInt32 s_iParamOffset = 1; // should be one, to start counting at 0, instead of 1
};
