...
bool wdLuaWrapper::IsVariableAvailable(const char* szName) const
{
...
bool wdLuaWrapper::IsFunctionAvailable(const char* szFunction) const
{
  if (m_States.m_iOpenTables == 0)
...
wdInt32 wdLuaWrapper::GetIntVariable(const char* szName, wdInt32 iDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
bool wdLuaWrapper::GetBoolVariable(const char* szName, bool bDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
float wdLuaWrapper::GetFloatVariable(const char* szName, float fDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
const char* wdLuaWrapper::GetStringVariable(const char* szName, const char* szDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
WD_STATICLINK_FILE(Core, Core_Scripting_LuaWrapper_Variables);
