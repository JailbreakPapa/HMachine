...
public:
  ...
  virtual void StartFrameCapture(wdWindowHandle hWnd) = 0;
  ...
  virtual void EndFrameCaptureAndWriteOutput(wdWindowHandle hWnd) = 0;
  ...
  virtual void EndFrameCaptureAndDiscardResult(wdWindowHandle hWnd) = 0;

  /// \brief Retrieve the absolute file name of the last successful frame capture. Returns WD_FAILURE if no successful capture has
  /// been performed.
  virtual wdResult GetLastAbsCaptureFileName(wdStringBuilder& out_sFileName) const = 0;
};
