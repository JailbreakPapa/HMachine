...
using wdWindowHandle = wdMinWindows::HWND;
using wdWindowInternalHandle = GLFWwindow*;
#    define INVALID_WINDOW_HANDLE_VALUE (wdWindowHandle)(0)
...
struct wdXcbWindowHandle
{
  xcb_connection_t* m_pConnection;
  wdUInt32 m_Window;
};

struct wdWindowHandle
{
  enum class Type
  {
    Invalid = 0,
  ...
  union
  {
    ...
    wdXcbWindowHandle xcbWindow;
  };

  bool operator==(wdWindowHandle& rhs)
  {
    if (type != rhs.type)
      return false;
...
using wdWindowInternalHandle = wdWindowHandle;
#    define INVALID_WINDOW_HANDLE_VALUE \
      wdWindowHandle {}
#  else
using wdWindowHandle = GLFWwindow*;
using wdWindowInternalHandle = GLFWwindow*;
...
using wdWindowHandle = IUnknown*;
using wdWindowInternalHandle = wdWindowHandle;
#  define INVALID_WINDOW_HANDLE_VALUE nullptr

#else

using wdWindowHandle = void*;
using wdWindowInternalHandle = wdWindowHandle;
#  define INVALID_WINDOW_HANDLE_VALUE nullptr
...
class WD_CORE_DLL wdWindowBase
{
public:
  virtual ~wdWindowBase() = default;

  virtual wdSizeU32 GetClientAreaSize() const = 0;
  virtual wdWindowHandle GetNativeWindowHandle() const = 0;
...
struct WD_CORE_DLL wdWindowMode
{
...
struct WD_CORE_DLL wdWindowCreationDesc
{
  ...
  ///
  /// This function can only fail if wdScreen::EnumerateScreens fails to enumerate the available screens.
  wdResult AdjustWindowSizeAndPosition();

  /// Serializes the configuration to DDL.
  void SaveToDDL(wdOpenDdlWriter& ref_writer);

  /// Serializes the configuration to DDL.
  wdResult SaveToDDL(wdStringView sFile);

  /// Deserializes the configuration from DDL.
  void LoadFromDDL(const wdOpenDdlReaderElement* pParentElement);

  /// Deserializes the configuration from DDL.
  wdResult LoadFromDDL(wdStringView sFile);


  /// The window title to be displayed.
  wdString m_Title = "wdEngine";

  /// Defines how the window size is determined.
  wdEnum<wdWindowMode> m_WindowMode;

  /// The monitor index is as given by wdScreen::EnumerateScreens.
  ...
  wdVec2I32 m_Position = wdVec2I32(0x80000000, 0x80000000); // Magic number on windows that positions the window at a 'good default position'

  /// The pixel resolution of the window.
  wdSizeU32 m_Resolution = wdSizeU32(1280, 720);
...
public:
  ...
  inline const wdWindowCreationDesc& GetCreationDescription() const { return m_CreationDescription; }

  /// \brief Returns the size of the client area / ie. the window resolution.
  virtual wdSizeU32 GetClientAreaSize() const override { return m_CreationDescription.m_Resolution; }

  /// \brief Returns the platform specific window handle.
  virtual wdWindowHandle GetNativeWindowHandle() const override;
  ...
  virtual bool IsFullscreenWindow(bool bOnlyProperFullscreenMode = false) const override
  {
    if (bOnlyProperFullscreenMode)
      return m_CreationDescription.m_WindowMode == wdWindowMode::FullscreenFixedResolution;

    return wdWindowMode::IsFullscreen(m_CreationDescription.m_WindowMode);
  }
  ...
  ///
  /// Will automatically call wdWindow::Destroy if window is already initialized.
  ///
  /// \see wdWindow::Destroy, wdWindow::Initialize
  wdResult Initialize();
  ...
  ///
  /// \see wdWindow::Destroy, wdWindow::Initialize
  wdResult Initialize(const wdWindowCreationDesc& creationDescription)
  {
  ...
  wdResult Resize(const wdSizeU32& newWindowSize);
  ...
  /// \see OnWindowMessage
  virtual void OnResize(const wdSizeU32& newWindowSize);

  /// \brief Called when the window position is changed. Not possible on all OSes.
  virtual void OnWindowMove(const wdInt32 iNewPosX, const wdInt32 iNewPosY) {}
  ...
  ///
  /// \see OnResizeMessage
  virtual void OnWindowMessage(wdMinWindows::HWND pWnd, wdMinWindows::UINT msg, wdMinWindows::WPARAM wparam, wdMinWindows::LPARAM lparam);
...
#else
#  error "Missing code for wdWindow on this platform!"
#endif

  /// \brief Returns the input device that is attached to this window and typically provides mouse / keyboard input.
  wdStandardInputDevice* GetInputDevice() const { return m_pInputDevice.Borrow(); }

  /// \brief Returns a number that can be used as a window number in wdWindowCreationDesc
  ///
  /// This number just increments every time an wdWindow is created. It starts at zero.
  static wdUInt8 GetNextUnusedWindowNumber();

protected:
  /// Description at creation time. wdWindow will not update this in any method other than Initialize.
  /// \remarks That means that messages like Resize will also have no effect on this variable.
  wdWindowCreationDesc m_CreationDescription;

private:
  bool m_bInitialized = false;

  wdUniquePtr<wdStandardInputDevice> m_pInputDevice;

  mutable wdWindowInternalHandle m_hWindowHandle = wdWindowInternalHandle();
  ...
  /// increased every time an wdWindow is created, to be able to get a free window index easily
  static wdUInt8 s_uiNextUnusedWindowNumber;
