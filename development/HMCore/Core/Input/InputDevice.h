...
///
/// All input handling is usually handled through the wdInputManager class.
...
///
/// By deriving from wdInputDevice you can extend what hardware the engine supports. The derived class should override
...
class WD_CORE_DLL wdInputDevice : public wdEnumerable<wdInputDevice, wdReflectedClass>
{
  WD_DECLARE_ENUMERABLE_CLASS_WITH_BASE(wdInputDevice, wdReflectedClass);
  WD_ADD_DYNAMIC_REFLECTION(wdInputDevice, wdReflectedClass);
...
private:
  ...
  /// on Windows the WM_CHAR message sends this information. An wdInputDevice derived class should never try to compute this
  /// itself, but instead query this information from the OS, which will also handle localization.
  wdUInt32 RetrieveLastCharacter();

  /// \brief Calls UpdateHardwareState() on all devices.
  static void UpdateAllHardwareStates(wdTime tTimeDifference);
  ...
  static wdUInt32 RetrieveLastCharacterFromAllDevices();

protected:
  /// \brief Calls RegisterInputSlot() on the wdInputManager and passes the parameters through.
  static void RegisterInputSlot(const char* szName, const char* szDefaultDisplayName, wdBitflags<wdInputSlotFlags> SlotFlags); // [tested]
  ...
  wdMap<wdString, float> m_InputSlotValues; // [tested]

  /// \brief If this input device type handles character input, it should write the last typed character into this variable.
  /// The wdInputManager calls RetrieveLastCharacter() to query what the user typed last.
  wdUInt32 m_uiLastCharacter; // [tested]
  ...
  /// This will be the case when wdInputManager::PollHardware is used to make more frequent hardware updates
  ...
  /// \brief Override this, if you need to reset certain input slot values to zero, after the wdInputManager is finished with the current
  /// frame update.
  ...
  /// \brief This function is called once after wdInputManager::Update with the same time delta value.
  /// It allows to update hardware state, such as the vibration of gamepad motors.
  virtual void UpdateHardwareState(wdTime tTimeDifference) {}
