...
///
/// This class is derived from wdInputDevice but adds some interface functions common to most controllers.
...
class WD_CORE_DLL wdInputDeviceController : public wdInputDevice
{
  WD_ADD_DYNAMIC_REFLECTION(wdInputDeviceController, wdInputDevice);
...
public:
  ...
  void EnableVibration(wdUInt8 uiVirtual, bool bEnable);

  /// \brief Checks whether vibration is enabled on the given controller (virtual index).
  bool IsVibrationEnabled(wdUInt8 uiVirtual) const;
  ...
  void SetVibrationStrength(wdUInt8 uiVirtual, Motor::Enum motor, float fValue);

  /// \brief Returns the amount of (constant) vibration that is currently set on this controller.
  float GetVibrationStrength(wdUInt8 uiVirtual, Motor::Enum motor);
  ...
  void SetControllerMapping(wdUInt8 uiVirtualController, wdInt8 iTakeInputFromPhysical);
  ...
  wdInt8 GetControllerMapping(wdUInt8 uiVirtual) const;

  /// \brief Queries whether the controller with the given physical index is connected to the computer.
  /// This may change at any time.
  virtual bool IsControllerConnected(wdUInt8 uiPhysical) const = 0;
  ...
  void AddVibrationTrack(wdUInt8 uiVirtual, Motor::Enum motor, float* pVibrationTrackValue, wdUInt32 uiSamples, float fScalingFactor = 1.0f);
...
protected:
  ...
  void UpdateVibration(wdTime tTimeDifference);
...
private:
  ...
  virtual void ApplyVibration(wdUInt8 uiPhysicalController, Motor::Enum eMotor, float fStrength) = 0;

  wdUInt32 m_uiVibrationTrackPos;
  float m_fVibrationTracks[MaxControllers][Motor::ENUM_COUNT][MaxVibrationSamples];
  bool m_bVibrationEnabled[MaxControllers];
  wdInt8 m_iControllerMapping[MaxControllers];
