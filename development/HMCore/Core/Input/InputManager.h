...
struct WD_CORE_DLL wdInputActionConfig
{
  ...
  /// then that is an absolute movement and it does not depend on how much time elapsed. Therefore the wdInputManager will take care NOT to
  ...
  wdString m_sInputSlotTrigger[MaxInputSlotAlternatives];
  ...
  /// exponential scaling (i.e SlotValue = wdMath::Pow(SlotValue, -ScaleValue)). Default is 1.0f.
  ...
  wdString m_sFilterByInputSlotX[MaxInputSlotAlternatives];
  ...
  wdString m_sFilterByInputSlotY[MaxInputSlotAlternatives];
  ...
  enum OnLeaveArea
  {
    LoseFocus, ///< The input action will lose focus and thus get 'deactivated' immediately. Ie. it will return wdKeyState::Released.
    KeepFocus, ///< The input action will keep focus and continue to return wdKeyState::Down, until all trigger slots are actually released.
  };
  ...
  enum OnEnterArea
  {
    ActivateImmediately, ///< The input action will immediately get activated and return wdKeyState::Pressed, even though the input slots
...
///
/// The wdInputManager is the central hub through which you can configure which keys will trigger which actions. You can query in which
...
class WD_CORE_DLL wdInputManager
{
public:
  ...
  static void Update(wdTime timeDifference); // [tested]
  ...
  ///
  /// If iTrigger is set, the name of that trigger (0 .. wdInputActionConfig::MaxInputSlotAlternatives) will be used.
  ...
  static const char* GetInputSlotDisplayName(const char* szInputSet, const char* szAction, wdInt32 iTrigger = -1);
  ...
  static wdBitflags<wdInputSlotFlags> GetInputSlotFlags(const char* szInputSlot); // [tested]
  ...
  static wdKeyState::Enum GetInputSlotState(const char* szInputSlot, float* pValue = nullptr); // [tested]

  /// \brief Returns an array that contains all the names of all currently known input slots.
  static void RetrieveAllKnownInputSlots(wdDynamicArray<const char*>& out_inputSlots);
  ...
  /// with the value.
  static wdUInt32 RetrieveLastCharacter(bool bResetCurrent = true); // [tested]
  ...
  ///   wdInputManager do the frame time scaling, because it should not be applied to all input, e.g. mouse delta values should never be
  ...
  static void SetInputActionConfig(const char* szInputSet, const char* szAction, const wdInputActionConfig& config,
  ...
  /// does not exist.
  static wdInputActionConfig GetInputActionConfig(const char* szInputSet, const char* szAction); // [tested]
  ...
  static wdKeyState::Enum GetInputActionState(const char* szInputSet, const char* szAction, float* pValue = nullptr,
    wdInt8* pTriggeredSlot = nullptr); // [tested]
  ...
  static const wdString GetActionDisplayName(const char* szAction); // [tested]

  /// \brief Returns the names of all currently registered input sets.
  static void GetAllInputSets(wdDynamicArray<wdString>& out_inputSetNames); // [tested]

  /// \brief Returns the names of all input actions in the given input set.
  static void GetAllInputActions(const char* szInputSetName, wdDynamicArray<wdString>& out_inputActions); // [tested]
  ...
  ///
  /// Note that when the input is injected after wdInputManager::Update was called, its effect will be delayed by one frame.
  ...
  static const char* GetPressedInputSlot(wdInputSlotFlags::Enum mustHaveFlags, wdInputSlotFlags::Enum mustNotHaveFlags); // [tested]

  /// \brief Mostly for internal use. Converts a scan-code value to the string that is used inside the engine for that key.
  static const char* ConvertScanCodeToEngineName(wdUInt8 uiScanCode, bool bIsExtendedKey);
  ...
  using wdEventInput = wdEvent<const InputEventData&>;

  /// \brief Adds an event handler that is called for input events.
  static wdEventSubscriptionID AddEventHandler(wdEventInput::Handler handler) { return s_InputEvents.AddEventHandler(handler); }

  /// \brief Removes a previously added event handler.
  static void RemoveEventHandler(wdEventInput::Handler handler) { s_InputEvents.RemoveEventHandler(handler); }
  static void RemoveEventHandler(wdEventSubscriptionID id) { s_InputEvents.RemoveEventHandler(id); }

private:
  friend class wdInputDevice;
  ...
  static void RegisterInputSlot(const char* szName, const char* szDefaultDisplayName, wdBitflags<wdInputSlotFlags> SlotFlags);
...
private:
  ...
  struct wdInputSlot
  {
    ...
    wdString m_sDisplayName;                  ///< The display name. Use this to present input slots in UIs.
    float m_fValue;                           ///< The current value.
    wdKeyState::Enum m_State;                 ///< The current state.
    float m_fDeadZone;                        ///< The dead zone. Unless the value exceeds this, it reports a zero value.
    wdBitflags<wdInputSlotFlags> m_SlotFlags; ///< Describes the capabilities of the slot.
  };
  ...
  struct wdActionData
  {
    ...
    wdInputActionConfig m_Config; ///< The configuration that was specified by the user.

    float m_fValue;           ///< The current value. This is the maximum of all input slot values that trigger this action.
    wdKeyState::Enum m_State; ///< The current state. Derived from m_fValue.

    wdInt8 m_iTriggeredViaAlternative;
  };

  using wdActionMap = wdMap<wdString, wdActionData>;    ///< Maps input action names to their data.
  using wdInputSetMap = wdMap<wdString, wdActionMap>;   ///< Maps input set names to their data.
  using wdInputSlotsMap = wdMap<wdString, wdInputSlot>; ///< Maps input slot names to their data.

  /// \brief The internal data of the wdInputManager. Not allocated until it is actually required.
  struct InternalData
  {
    wdInputSetMap s_ActionMapping;                  ///< Maps input set names to their data.
    wdInputSlotsMap s_InputSlots;                   ///< Maps input slot names to their data.
    wdMap<wdString, wdString> s_ActionDisplayNames; ///< Stores a display name for each input action.
    wdMap<wdString, float> s_InjectedInputSlots;
  };

  /// \brief The last (Unicode) character that was typed by the user, as reported by the OS (on Windows: WM_CHAR).
  static wdUInt32 s_uiLastCharacter;

  static bool s_bInputSlotResetRequired;

  /// \brief If not empty, all input for other input sets is returned as inactive ('key up')
  static wdString s_sExclusiveInputSet;
  ...
  static void UpdateInputActions(wdTime tTimeDifference);

  /// \brief Updates the state of all input actions in the given input set.
  static void UpdateInputActions(const char* szInputSet, wdActionMap& Actions, wdTime tTimeDifference);
  ...
  static wdActionMap::Iterator GetBestAction(wdActionMap& Actions, const wdString& sSlot, const wdActionMap::Iterator& itFirst);

private:
  WD_MAKE_SUBSYSTEM_STARTUP_FRIEND(Core, InputManager);

  static void DeallocateInternals();
  static InternalData& GetInternals();

  static wdEventInput s_InputEvents;
